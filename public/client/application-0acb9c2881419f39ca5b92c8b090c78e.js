...
(function() {
  jQuery.fn.focusToEnd = function() {
    return this.each(function() {
      var value;
      value = $(this).val();
      return $(this).focus().val('').val(value);
    });
  };

  jQuery.fn.insertAtCaret = function(myValue) {
    return this.each(function(i) {
      var endPos, scrollTop, startPos, _ref;
      if (document.selection) {
        this.focus();
        document.selection.createRange().text = myValue;
        this.focus();
      } else if (this.selectionStart || this.selectionStart === '0') {
        _ref = [this.selectionStart, this.selectionEnd, this.scrollTop], startPos = _ref[0], endPos = _ref[1], scrollTop = _ref[2];
...
(function() {
  ...
  String.prototype.camel = function() {
    return this.replace(/(_[^_]+)/g, function(match) {
      return match.slice(1).capitalize();
    });
  };

  String.prototype.underscore = function() {
    var str;
    str = this.replace(/([A-Z])/g, function(match) {
  ...
  window.Nali = {
    ...
    starting: function() {
      ...
      for (name in _ref) {
        ...
        this.starting.call(extension);
      }
      return this;
    },
    extend: function(params) {
      var name, param, _results;
      _results = [];
      for (name in params) {
        param = params[name];
        param._name = name;
        this[name] = this.extensions[name] = this.child(param);
        _results.push(this[name].extensions = {});
      }
      return _results;
    },
    ...
    expand: function(obj) {
      ...
      if (obj instanceof Object) {
        ...
        console.error("Expand of %O error - argument is not Object", this);
      }
      return this;
    },
    copy: function(obj) {
    ...
    childOf: function(parent) {
      ...
      if (parent instanceof Object) {
        ...
        if (((_ref = this.prototype) != null ? _ref._name : void 0) == null) {
          return false;
        }
        if (this.prototype._name === parent) {
          return true;
        }
      }
      return this.prototype.childOf(parent);
    },
    runExtensions: function(context) {
      var _ref;
      if (context == null) {
        context = this;
      }
      if ((_ref = this.prototype) != null) {
        _ref.runExtensions(context);
      }
      if (this.hasOwnProperty('extension')) {
        return this.extension.call(context);
      }
    },
    runCloning: function(context) {
      var _ref;
      if (context == null) {
        context = this;
      }
      if ((_ref = this.prototype) != null) {
        _ref.runCloning(context);
      }
      if (this.hasOwnProperty('cloning')) {
        return this.cloning.call(context);
      }
    },
    getter: function(property, callback) {
      this.__defineGetter__(property, callback);
      return this;
    },
    setter: function(property, callback) {
    ...
    access: function(obj) {
      ...
      _fn = (function(_this) {
        return function(property) {
          _this.getter(property, function() {
            return obj[property];
          });
          return _this.setter(property, function(value) {
    ...
    destroyObservation: function() {
      this.unsubscribeAll();
      this.unsubscribeFromAll();
      return this;
    },
    addObservationItem: function(to, obj, event, callback) {
      var item, _i, _len, _ref, _ref1;
      _ref = this[to];
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        item = _ref[_i];
        if (item[0] === obj && item[1] === event && ((_ref1 = item[2]) === (void 0) || _ref1 === callback)) {
          return this;
        }
      }
      this[to].push(callback ? [obj, event, callback] : [obj, event]);
      return this;
    },
    removeObservationItem: function(from, obj, event) {
      var item, _i, _len, _ref;
      _ref = this[from].slice(0);
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        item = _ref[_i];
        if (item[0] === obj && (item[1] === event || !event)) {
          this[from].splice(this[from].indexOf(item), 1);
        }
      }
      return this;
    },
    subscribe: function(observer, event, callback) {
      this.addObservationItem('observers', observer, event, callback);
      observer.addObservationItem('observables', this, event, callback);
      return this;
    },
    subscribeTo: function(observable, event, callback) {
      observable.subscribe(this, event, callback);
      return this;
    },
    subscribeOne: function(observer, event, callback) {
      var callbackOne;
      callbackOne = (function(_this) {
        return function() {
          var args;
          args = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
          callback.call.apply(callback, [observer].concat(__slice.call(args)));
          return _this.unsubscribe(observer, event);
        };
      })(this);
      this.subscribe(observer, event, callbackOne);
      return this;
    },
    subscribeOneTo: function(observable, event, callback) {
      observable.subscribeOne(this, event, callback);
      return this;
    },
    unsubscribe: function(observer, event) {
      this.removeObservationItem('observers', observer, event);
      observer.removeObservationItem('observables', this, event);
      return this;
    },
    unsubscribeFrom: function(observable, event) {
      observable.unsubscribe(this, event);
      return this;
    },
    unsubscribeAll: function(event) {
      var item, _i, _len, _ref;
      _ref = this.observers.slice(0);
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        item = _ref[_i];
        this.unsubscribe(item[0], event);
      }
      return this;
    },
    unsubscribeFromAll: function(event) {
      var item, _i, _len, _ref;
      _ref = this.observables.slice(0);
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        item = _ref[_i];
        this.unsubscribeFrom(item[0], event);
      }
      return this;
    },
    trigger: function() {
      var args, event, item, _i, _len, _ref, _ref1;
      event = arguments[0], args = 2 <= arguments.length ? __slice.call(arguments, 1) : [];
      _ref = this.observers.slice(0);
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        item = _ref[_i];
        if (item[1] === event) {
          (_ref1 = item[2]).call.apply(_ref1, [item[0]].concat(__slice.call(args)));
        }
      }
      return this;
    }
  ...
  Nali.extend({
    Application: {
      ...
      run: function(options) {
        var key, value;
        this.prototype.starting();
        for (key in options) {
          value = options[key];
          this[key] = value;
        }
        return this.onReadyDOM(function() {
          this.prototype._ = this.domEngine;
          this.htmlContainer = this._(this.htmlContainer);
          this.setTitle(this.title);
          this.Router.start();
          return this.runConnection();
        });
      },
      onReadyDOM: function(callback) {
      ...
      runConnection: function() {
        if (this.useWebSockets) {
          this.Connection.subscribe(this, 'open', this.onConnectionOpen);
          this.Connection.subscribe(this, 'close', this.onConnectionClose);
          this.Connection.subscribe(this, 'error', this.onConnectionError);
      ...
      setTitle: function(title) {
        ...
        if (this.titleBox == null) {
          this.titleBox = (exists = this._('head title')).lenght ? exists : this._('<title>').appendTo('head');
        }
        this.titleBox[0].innerText = this.title;
...
(function() {
  ...
  Nali.extend({
    Connection: {
      initialize: function() {
        this.prototype.expand({
          query: (function(_this) {
            return function() {
              var args;
              args = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
              return _this.query.apply(_this, args);
            };
          })(this)
        });
        return this;
      ...
      open: function() {
        this.dispatcher = new WebSocket(this.Application.wsServer);
        this.dispatcher.onopen = (function(_this) {
          return function(event) {
            return _this.identification();
          };
        })(this);
        this.dispatcher.onclose = (function(_this) {
          return function(event) {
            return _this.onClose(event);
          };
        })(this);
        this.dispatcher.onerror = (function(_this) {
          return function(event) {
            return _this.onError(event);
          };
        })(this);
        this.dispatcher.onmessage = (function(_this) {
          return function(event) {
            return _this.onMessage(JSON.parse(event.data));
          };
        })(this);
        this.keepAlive();
        return this;
      ...
      onError: function(event) {
        return console.warn('Connection error %O', event);
      },
      onClose: function(event) {
        ...
        })(this)), this.reconnectDelay * 100);
        return this.reconnectDelay += 1;
      },
      onMessage: function(message) {
        return this[message.action](message);
      },
      send: function(msg) {
        this.dispatcher.send(JSON.stringify(msg));
        return this;
      },
      keepAlive: function() {
        ...
        if (this.Application.keepAliveDelay) {
          this.keepAliveTimer = setTimeout((function(_this) {
            return function() {
          ...
          })(this), this.Application.keepAliveDelay * 1000);
        }
        return this;
      },
      pong: function() {
      ...
      identification: function() {
        this.send({
          nali_browser_id: this.Cookie.get('nali_browser_id') || this.Cookie.set('nali_browser_id', this.Model.guid())
        });
        return this;
      },
      sync: function(message) {
        this.Model.sync(message.params);
        return this;
      },
      appRun: function(_arg) {
        var method, params, _base;
        method = _arg.method, params = _arg.params;
        if (typeof (_base = this.Application)[method] === "function") {
          _base[method](params);
        }
        return this;
      },
      callMethod: function(_arg) {
        var id, method, model, params, _ref;
        model = _arg.model, method = _arg.method, params = _arg.params;
        if (model === 'Notice') {
          this.Notice[method](params);
        } else {
          _ref = model.split('.'), model = _ref[0], id = _ref[1];
          this.Model.callStackAdd({
            model: model,
            id: id,
            method: method,
            params: params
      ...
      success: function(message) {
        var _base;
        if (typeof (_base = this.journal[message.journal_id]).success === "function") {
          _base.success(message.params);
        }
        delete this.journal[message.journal_id];
        return this;
      },
      failure: function(message) {
        var _base;
        if (typeof (_base = this.journal[message.journal_id]).failure === "function") {
          _base.failure(message.params);
        }
        delete this.journal[message.journal_id];
        return this;
      },
      query: function(to, params, success, failure) {
        var action, callbacks, controller, _ref;
        if (!this.Application.useWebSockets) {
          return typeof success === "function" ? success() : void 0;
        }
        _ref = to.split('.'), controller = _ref[0], action = _ref[1];
        this.journal.push(callbacks = {
          success: success,
          failure: failure
        });
        this.send({
          ...
          action: action,
          params: params,
          journal_id: this.journal.indexOf(callbacks)
        });
        return this;
      }
    }
...
(function() {
  ...
  Nali.extend({
    Router: {
        ...
        this.prototype.expand({
          redirect: (function(_this) {
            return function() {
              var args;
              args = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
              return _this.redirect.apply(_this, args);
            };
          })(this)
        });
        return this;
      ...
      start: function() {
        this.scanRoutes();
        this._(window).on('popstate', (function(_this) {
          return function(event) {
            event.preventDefault();
            event.stopPropagation();
            _this.saveHistory(false);
            return _this.redirect(event.target.location.pathname);
          };
        })(this));
        return this;
      },
      scanRoutes: function() {
        var controller, name, route, _ref;
        _ref = this.Controller.extensions;
        for (name in _ref) {
      ...
      redirect: function(url, options) {
        var action, controller, filters, found, name, params, value, _i, _len;
        if (url == null) {
        ...
        if (found = this.findRoute(this.prepare(url) || this.prepare(this.Application.defaultUrl))) {
          controller = found.controller, action = found.action, filters = found.filters, params = found.params;
          for (value = _i = 0, _len = options.length; _i < _len; value = ++_i) {
            name = options[value];
            params[name] = value;
          }
          controller.run(action, filters, params);
        } else if (this.Application.notFoundUrl) {
          this.redirect(this.Application.notFoundUrl);
        } else {
          console.warn("Not exists route to the address %s", url);
        }
        return this;
      },
      ...
      findRoute: function(url) {
        var action, controller, filters, match, name, params, route, segments, _i, _j, _len, _len1, _ref, _ref1, _ref2, _ref3;
        _ref = this.routes;
        for (route in _ref) {
          controller = _ref[route];
          if (!(match = url.match(new RegExp(route, 'i')))) {
            continue;
          }
          segments = (this.routedUrl = url).split('/').slice(1);
          if (_ref1 = segments[0], __indexOf.call(Object.keys(controller._actions), _ref1) >= 0) {
            action = segments.shift();
          } else if (!(action = controller.actions["default"])) {
            console.error('Unspecified controller action');
          }
          filters = {};
          _ref2 = controller._actions[action].filters;
          ...
          params = {};
          _ref3 = controller._actions[action].params;
          for (_j = 0, _len1 = _ref3.length; _j < _len1; _j++) {
            name = _ref3[_j];
            params[name] = segments[0] != null ? segments.shift() : null;
          }
          return {
            controller: controller,
            action: action,
      ...
      changeUrl: function(url) {
        ...
        if (this.saveHistory()) {
          ...
          if (this.routedUrl !== this.url) {
            history.pushState(null, null, '/' + (this.url = this.routedUrl));
          }
        } else {
...
(function() {
  ...
  Nali.extend({
    View: {
      extension: function() {
        if (this._name !== 'View') {
          this._shortName = this._name.underscore().split('_').slice(1).join('_').camel();
          this.parseTemplate();
          this.parseEvents();
        }
        return this;
      ...
      onSourceDestroyed: function() {
        return this.hide();
      },
      getOf: function(source, property) {
        this.redrawOn(source, "update." + property);
        return source[property];
      },
      getMy: function(property) {
        return this.getOf(this.model, property);
      },
      redrawOn: function(source, event) {
        return this.subscribeTo(source, event, this.onSourceUpdated);
      },
      insertTo: function() {
        var layout, _ref;
        if ((_ref = (layout = this.layout())) != null ? typeof _ref.childOf === "function" ? _ref.childOf('View') : void 0 : void 0) {
          return layout.show()["yield"];
        } else {
          return this.Application.htmlContainer;
        }
      },
      draw: function() {
        this.runAssistants('draw');
        if (typeof this.onDraw === "function") {
          this.onDraw();
        }
        return this;
      },
      show: function(insertTo) {
        if (insertTo == null) {
          insertTo = this.insertTo();
        }
        this.prepareElement();
        if (!this.visible) {
          this.runModelCallback('beforeShow');
          this.draw().bindEvents();
          this.runAssistants('show');
          this.subscribeTo(this.model, 'destroy', this.onSourceDestroyed);
          this.element.appendTo(insertTo);
          if (this.onShow != null) {
            setTimeout(((function(_this) {
              return function() {
                return _this.onShow();
          ...
          this.trigger('show');
          this.visible = true;
          this.runModelCallback('afterShow');
        } else {
          this.draw();
        }
        return this;
      ...
      hide: function(delay) {
        ...
        if (this.visible) {
          this.runModelCallback('beforeHide');
          if (typeof this.onHide === "function") {
            this.onHide();
          }
          this.trigger('hide');
          this.runAssistants('hide');
          this.hideElement(delay || this.hideDelay);
          this.destroyObservation();
          this.visible = false;
          this.runModelCallback('afterHide');
        }
        return this;
      },
      hideElement: function(delay) {
      ...
      runModelCallback: function(type) {
        var _ref, _ref1;
        if ((_ref = this.model[type]) != null) {
          if ((_ref1 = _ref[this._shortName]) != null) {
            _ref1.call(this.model);
          }
        }
        return this;
      },
      runLink: function(event) {
        event.preventDefault();
        this.runUrl(event.currentTarget.getAttribute('href'));
        return this;
      },
      runForm: function(event) {
        event.preventDefault();
        this.runUrl(event.currentTarget.getAttribute('action'), form2js(event.currentTarget, '.', false));
        return this;
      },
      runUrl: function(url, params) {
        var args, chain, match, method, result, segments, source, _ref;
        if (match = url.match(/^(@@?)(.+)/)) {
          _ref = match[2].split('/'), chain = _ref[0], segments = 2 <= _ref.length ? __slice.call(_ref, 1) : [];
          if (result = this.analizeChain(chain, (match[1].length === 1 ? this : this.model))) {
            source = result[0], method = result[1];
            if (method in source) {
              args = this.parseUrlSegments(segments);
              if (params) {
                args.unshift(params);
              }
              source[method].apply(source, args);
            } else {
              console.warn("Method %s not exists of %O", method, source);
            }
          }
        } else {
      ...
      parseUrlSegments: function(segments) {
        var last, name, params, segment, value, _i, _len, _ref;
        params = [];
        for (_i = 0, _len = segments.length; _i < _len; _i++) {
          ...
          _ref = segment.split(':'), name = _ref[0], value = _ref[1];
          if (value) {
      ...
      parseEvents: function() {
        var error, event, events, handlers, other, selector, type, _i, _len, _ref, _ref1, _ref2;
        this.eventsMap = [];
        if (this.events) {
          ...
          for (_i = 0, _len = _ref.length; _i < _len; _i++) {
            event = _ref[_i];
            try {
              _ref1 = event.split(/\s+(on|one)\s+/), handlers = _ref1[0], type = _ref1[1], other = _ref1[2];
              _ref2 = other.split(/\s+at\s+/), events = _ref2[0], selector = _ref2[1];
              handlers = handlers.split(/\s*,\s*/);
              events = events.replace(/\s*,\s*/, ' ');
              if (!(type && events.length && handlers.length)) {
                throw true;
              }
            } catch (_error) {
              console.warn("Events parsing error: \"%s\" of %O", event, this);
              error = true;
            }
            if (error) {
              error = false;
            } else {
              this.eventsMap.push([selector, type, events, handlers]);
            }
          }
        }
        return this;
      ...
      bindEvents: function() {
        var events, handler, handlers, selector, type, _fn, _i, _j, _len, _len1, _ref, _ref1;
        if (this._eventsBinded == null) {
          this.element.find('a').on('click', (function(_this) {
            return function(event) {
              return _this.runLink(event);
            };
          })(this));
          this.element.find('form').on('submit', (function(_this) {
            return function(event) {
              return _this.runForm(event);
            };
          })(this));
          this.element.on('click', (function(_this) {
            return function(event) {
              if (_this.element.is('a')) {
                return _this.runLink(event);
              }
            };
          })(this));
          this.element.on('submit', (function(_this) {
            return function(event) {
          ...
          _ref = this.eventsMap;
          for (_i = 0, _len = _ref.length; _i < _len; _i++) {
            _ref1 = _ref[_i], selector = _ref1[0], type = _ref1[1], events = _ref1[2], handlers = _ref1[3];
            _fn = (function(_this) {
              return function(selector, type, events, handler) {
                return _this.element[type](events, selector, function(event) {
            ...
            for (_j = 0, _len1 = handlers.length; _j < _len1; _j++) {
              handler = handlers[_j];
              _fn(selector, type, events, handler);
            }
          }
      ...
      prepareElement: function() {
        if (!this.element) {
          this.element = this._(this.template);
      ...
      parseTemplate: function() {
        var container;
        if (container = document.querySelector('#' + this._name.underscore())) {
          this.template = container.innerHTML.trim().replace(/\s+/g, ' ').replace(/({\s*\+.+?\s*})/g, ' <assist>$1</assist>');
          if (!RegExp("^<[^>]+" + this._name).test(this.template)) {
            this.template = "<div class=\"" + this._name + "\">" + this.template + "</div>";
          }
          this.parseAssistants();
          container.parentNode.removeChild(container);
        } else {
          console.warn('Template %s not exists', this._name);
        }
        return this;
      },
      parseAssistants: function() {
        var tmp;
        this.assistantsMap = [];
        if (/{\s*.+?\s*}|bind=".+?"/.test(this.template)) {
          tmp = document.createElement('div');
          tmp.innerHTML = this.template;
          this.scanAssistants(tmp.children[0]);
        }
        return this;
      },
      scanAssistants: function(node, path) {
        var attribute, child, index, _i, _j, _len, _len1, _ref, _ref1;
        if (path == null) {
          path = [];
        }
        if (node.nodeType === 3) {
          if (/{\s*yield\s*}/.test(node.textContent.trim()) && node.parentNode.childNodes.length === 1) {
            this.assistantsMap.push({
              nodepath: path,
              type: 'Yield'
            });
          } else if (/^{\s*\w+ of @\w*\s*}$/.test(node.textContent.trim()) && node.parentNode.childNodes.length === 1) {
            this.assistantsMap.push({
              nodepath: path,
              type: 'Relation'
            });
          ...
          if (node.attributes) {
            ...
            for (index = _i = 0, _len = _ref.length; _i < _len; index = ++_i) {
              ...
              if (attribute.name === 'bind') {
                ...
                this.assistantsMap.push({
                  nodepath: path.concat('attributes', index),
                  type: 'Attr'
                });
              }
            }
          ...
          for (index = _j = 0, _len1 = _ref1.length; _j < _len1; index = ++_j) {
            child = _ref1[index];
            this.scanAssistants(child, path.concat('childNodes', index));
          }
        }
        return this;
      },
      addAssistants: function() {
        var nodepath, type, _i, _len, _ref, _ref1;
        this.assistants = {
          show: [],
          draw: [],
          hide: []
        };
        _ref = this.assistantsMap;
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          _ref1 = _ref[_i], nodepath = _ref1.nodepath, type = _ref1.type;
          this["add" + type + "Assistant"](this.getNode(nodepath));
        }
        return this;
      },
      runAssistants: function(type) {
        var assistant, _i, _len, _ref;
        _ref = this.assistants[type];
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          assistant = _ref[_i];
          assistant.call(this);
        }
      ...
      addTextAssistant: function(node) {
        var initialValue;
        initialValue = node.textContent;
        this.assistants['draw'].push(function() {
          return node.textContent = this.analize(initialValue);
        });
        return this;
      },
      addAttrAssistant: function(node) {
        var initialValue;
        initialValue = node.value;
        this.assistants['draw'].push(function() {
          return node.value = this.analize(initialValue);
        });
        return this;
      },
      addHtmlAssistant: function(node) {
        var after, before, index, initialValue, parent;
        parent = node.parentNode;
        initialValue = node.innerHTML;
        index = Array.prototype.indexOf.call(parent.childNodes, node);
        after = parent.childNodes[index - 1] || null;
        before = parent.childNodes[index + 1] || null;
        this.assistants['draw'].push(function() {
          var element, end, start, _i, _j, _len, _len1, _ref, _ref1, _results;
          start = after ? Array.prototype.indexOf.call(parent.childNodes, after) + 1 : 0;
          end = before ? Array.prototype.indexOf.call(parent.childNodes, before) : parent.childNodes.length;
          _ref = Array.prototype.slice.call(parent.childNodes, start, end);
          for (_i = 0, _len = _ref.length; _i < _len; _i++) {
            node = _ref[_i];
            parent.removeChild(node);
          }
          _ref1 = this._(this.analize(initialValue));
          _results = [];
          for (_j = 0, _len1 = _ref1.length; _j < _len1; _j++) {
            element = _ref1[_j];
            _results.push(parent.insertBefore(element, before));
          }
          return _results;
        });
        return this;
      },
      addFormAssistant: function(node) {
        var bind, bindChange, property, setValue, source, updateSource, _node, _ref;
        if (bind = this.analizeChain(node.attributes.removeNamedItem('bind').value)) {
          source = bind[0], property = bind[1];
          _node = this._(node);
          updateSource = function() {
            var params;
            (params = {})[property] = node.type === 'checkbox' && !node.checked ? null : node.value;
          ...
          _ref = (function() {
            var _ref, _ref1;
            switch (false) {
              case (_ref = node.type) !== 'text' && _ref !== 'textarea':
                return [
                  function() {
                    return node.value = source[property];
                  }, function() {
                    return _node.on('change', (function(_this) {
                      return function() {
                        return updateSource.call(_this);
                      };
                    })(this));
                  }
                ];
              case (_ref1 = node.type) !== 'checkbox' && _ref1 !== 'radio':
                return [
                  function() {
                    return node.checked = source[property] + '' === node.value;
                  }, function() {
                    return _node.on('change', (function(_this) {
                      return function() {
                ...
                return [
                  function() {
                    ...
                    for (_i = 0, _len = node.length; _i < _len; _i++) {
                      option = node[_i];
                      if (source[property] + '' === option.value) {
          ...
          })(), setValue = _ref[0], bindChange = _ref[1];
          this.assistants['show'].push(function() {
            setValue.call(this);
            bindChange.call(this);
            return source.subscribe(this, "update." + property, (function(_this) {
              return function() {
                return setValue.call(_this);
              };
            })(this));
          });
          this.assistants['hide'].push(function() {
            return _node.off('change');
          });
        }
        return this;
      },
      addYieldAssistant: function(node) {
        return (this["yield"] = this._(node.parentNode))[0].removeChild(node);
      },
      addRelationAssistant: function(node) {
        var chain, insertTo, match, name, segments, _ref;
        _ref = node.textContent.match(/{\s*(\w+) of @(\w*)\s*}/), match = _ref[0], name = _ref[1], chain = _ref[2];
        (insertTo = node.parentNode).removeChild(node);
        segments = chain.length ? chain.split('.') : [];
        return this.assistants['show'].push(function() {
          var relation, view;
          if (relation = this.getSource(segments)) {
            if (relation.childOf('Collection')) {
              relation.show(name, insertTo, true);
              return relation.subscribeTo(this, 'hide', relation.reset);
            } else {
              view = relation.show(name, insertTo);
              return view.subscribeTo(this, 'hide', view.hide);
            }
          }
        });
      },
      analize: function(value) {
        return value.replace(/{\s*(.+?)\s*}/g, (function(_this) {
          return function(match, sub) {
            return _this.analizeMatch(sub);
          };
        })(this));
      },
      analizeMatch: function(sub) {
        var match, property, result, source, _ref, _ref1;
        if (match = sub.match(/^@([\w\.]+)(\?)?$/)) {
          if (result = this.analizeChain(match[1])) {
            source = result[0], property = result[1];
            if (typeof source.subscribe === "function") {
              source.subscribe(this, "update." + property, this.onSourceUpdated);
            }
            if (match[2] === '?') {
              if (source[property]) {
                return property;
          ...
          return (_ref = this.helpers) != null ? (_ref1 = _ref[match[1]]) != null ? _ref1.call(this) : void 0 : void 0;
        } else {
          return sub;
        }
      },
      getSource: function(segments, source) {
        var segment, _i, _len;
        if (source == null) {
          source = this.model;
        }
        for (_i = 0, _len = segments.length; _i < _len; _i++) {
          ...
          if (segment in source) {
            ...
            console.warn("%s: chain \"%s\" is invalid, segment \"%s\" not exists in %O", this._name, segments.join('.'), segment, source);
            return null;
          }
        }
        return source;
      },
      analizeChain: function(chain, source) {
        var property, segments;
        if (source == null) {
          source = this.model;
        }
        segments = chain.split('.');
        property = segments.pop();
        if (!(source = this.getSource(segments, source))) {
          return null;
        }
        return [source, property];
      }
  ...
  Nali.extend({
    Model: {
      extension: function() {
        var _base, _name;
        if (this._name !== 'Model') {
          this.table = (_base = this.tables)[_name = this._name] != null ? _base[_name] : _base[_name] = [];
          this.table.index = {};
          this.parseRelations();
          this.adapt();
        }
            ...
            (function(_this) {
              return (function(name, method) {
                if (typeof method === 'function') {
                  return _this[name.slice(1)] = function() {
                    var args;
                    args = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
      ...
      adaptViews: function() {
        var name, view, _ref;
        this.views = {};
        _ref = this.View.extensions;
        for (name in _ref) {
          view = _ref[name];
          if (name.indexOf(this._name) >= 0) {
            (function(_this) {
              return (function(name, view) {
                var hideMethod, short, shortCap, showMethod, viewMethod;
                _this.views[short = view._shortName] = view;
                shortCap = short.capitalize();
                if (_this[viewMethod = 'view' + shortCap] == null) {
                  _this[viewMethod] = function() {
                    return this.view(short);
                  };
                }
                if (_this[showMethod = 'show' + shortCap] == null) {
                  _this[showMethod] = function(insertTo) {
                    return this.show(short, insertTo);
                  };
                }
                if (_this[hideMethod = 'hide' + shortCap] == null) {
                  return _this[hideMethod] = function() {
                    return this.hide(short);
                  };
                }
              });
            })(this)(name, view);
      ...
      callStackAdd: function(params) {
        this.callStack.push(params);
        this.runStack();
        return this;
      },
      runStack: function() {
        var index, item, model, _i, _len, _ref;
        _ref = this.callStack.slice(0);
        for (index = _i = 0, _len = _ref.length; _i < _len; index = ++_i) {
          item = _ref[index];
          if (model = this.extensions[item.model].find(item.id)) {
            model[item.method](item.params);
            this.callStack.splice(this.callStack.indexOf(item), 1);
          }
        }
        return this;
      },
      accessing: function() {
        this.access(this.attributes);
        this.setRelations();
        return this;
      },
      save: function(success, failure) {
        ...
        if (this.isValid()) {
          this.query("" + (this._name.lower()) + "s.save", this.attributes, (function(_this) {
            return function(_arg) {
              var attributes, created, updated;
              attributes = _arg.attributes, created = _arg.created, updated = _arg.updated;
              _this.update(attributes, updated, created).write();
      ...
      sync: function(_arg) {
        var attributes, created, destroyed, model, updated, _name;
        _name = _arg._name, attributes = _arg.attributes, created = _arg.created, updated = _arg.updated, destroyed = _arg.destroyed;
        if (model = this.extensions[_name].find(attributes.id)) {
          if (destroyed) {
            model.remove();
          } else if (updated > this.updated) {
            model.updated = updated;
            model.created = created;
            model.update(attributes);
          }
        } else {
          model = this.extensions[_name]["new"](attributes);
          model.updated = updated;
          model.created = created;
          model.write();
        }
      ...
      select: function(options) {
        ...
        this.query(this._name.lower() + 's.select', obj);
        return this;
      },
      written: function() {
        return __indexOf.call(this.table, this) >= 0;
      },
      write: function() {
        if (this.id && !this.table.index[this.id]) {
          this.table.index[this.id] = this;
        }
        if (!this.written()) {
          ...
          this.Model.trigger("create." + (this._name.lower()), this);
          this.Model.runStack();
        }
        return this;
      },
      remove: function() {
        if (this.written()) {
          this.destroyed = true;
          delete this.table.index[this.id];
          this.table.splice(this.table.indexOf(this), 1);
          this.trigger('destroy');
          this.Model.trigger("destroy." + (this._name.lower()), this);
      ...
      "new": function(attributes) {
        ...
        for (name in attributes) {
          ...
          model[name] = this.normalizeValue(value);
        }
        return model;
      },
      create: function(attributes, success, failure) {
        return this["new"](attributes).save(success, failure);
      },
      update: function(attributes, checkValidation) {
        var changed, name, value;
        ...
        for (name in attributes) {
          value = attributes[name];
          if (this.updateProperty(name, value, checkValidation)) {
            changed.push(name);
          }
        }
        if (changed.length) {
          ...
          this.Model.trigger("update." + (this._name.lower()), this);
        }
        return this;
      },
      updateProperty: function(name, value, checkValidation) {
        var _name;
        if (checkValidation == null) {
          checkValidation = true;
        }
        value = this.normalizeValue(value);
        if (this[name] !== value && (!checkValidation || this.isValidAttributeValue(name, value))) {
          this[name] = value;
          if (typeof this[_name = 'onUpdate' + name.capitalize()] === "function") {
            this[_name]();
          }
      ...
      upgrade: function(attributes, success, failure) {
        this.update(attributes).save(success, failure);
        return this;
      },
      destroy: function(success, failure) {
        this.query(this._name.lower() + 's.destroy', this.attributes, success, failure);
        return this;
      },
      find: function(id) {
        return this.table.index[id];
      },
      where: function(filters) {
        return this.Collection["new"](this, filters);
      },
      all: function() {
        return this.where({
          id: /./
      ...
      guid: function() {
        ...
        return 'xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx'.replace(/[xy]/g, function(sub) {
          var rand;
          rand = (date + Math.random() * 16) % 16 | 0;
          date = Math.floor(date / 16);
          return (sub === 'x' ? rand : rand & 0x7 | 0x8).toString(16);
        });
      },
      defaultAttributes: function() {
        ...
        for (name in _ref) {
          ...
          if (value instanceof Object) {
            attributes[name] = value["default"] != null ? this.normalizeValue(value["default"]) : null;
          } else {
            attributes[name] = this.normalizeValue(value);
          }
        }
        return attributes;
      },
      ...
      isCorrect: function(filters) {
        var filter, name, result;
        if (filters == null) {
          filters = {};
        }
        ...
        for (name in filters) {
          filter = filters[name];
          result = name === 'correct' && typeof filter === 'function' ? filter.call(this) : this.isCorrectAttribute(this[name], filter);
          if (!result) {
            return false;
          }
        }
      ...
      isCorrectAttribute: function(attribute, filter) {
        var _ref;
        if (!attribute) {
          return false;
        }
        if (filter instanceof RegExp) {
          return filter.test(attribute);
          ...
          return (_ref = '' + attribute, __indexOf.call(filter, _ref) >= 0) || __indexOf.call(filter, +attribute) >= 0;
        } else {
          return false;
        }
      },
      parseRelations: function(type, options) {
        var params, section, _base, _i, _j, _len, _len1, _ref, _ref1;
        this.relations = {};
        _ref = ['belongsTo', 'hasOne', 'hasMany'];
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          type = _ref[_i];
          _ref1 = [].concat(this[type]);
          for (_j = 0, _len1 = _ref1.length; _j < _len1; _j++) {
            options = _ref1[_j];
            if (!(this[type] != null)) {
              continue;
            }
            params = this['parse' + type.capitalize()](this.parseInitialRelationParams(options));
            section = type + (params.through != null ? 'Through' : '');
            ((_base = this.relations)[section] != null ? _base[section] : _base[section] = []).push(params);
          }
        }
        return this;
      },
      parseInitialRelationParams: function(options) {
        var key, model, params, through;
        if (typeof options === 'object') {
          params = {
            name: Object.keys(options)[0]
          };
          if (through = options[params.name].through) {
            params.through = through;
          }
          if (key = options[params.name].key) {
            params.key = key;
          }
          if (model = options[params.name].model) {
            params.model = this.Model.extensions[model.capitalize()];
          }
        } else {
          params = {
            name: options
      ...
      parseBelongsTo: function(params) {
        if (params.model == null) {
          params.model = this.Model.extensions[params.name.capitalize()];
        }
        if (params.key == null) {
      ...
      parseHasOne: function(params) {
        if (params.model == null) {
          params.model = this.Model.extensions[params.name.capitalize()];
        }
        if (params.key == null) {
          params.key = (params.through ? params.name : this._name + '_id').lower();
        }
        return params;
      },
      parseHasMany: function(params) {
        if (params.model == null) {
          params.model = this.Model.extensions[params.name.slice(0, -1).capitalize()];
        }
        if (params.key == null) {
          params.key = (params.through ? params.name.slice(0, -1) : this._name + '_id').lower();
        }
        return params;
      },
      setRelations: function() {
        var type, _i, _len, _ref;
        _ref = ['belongsTo', 'hasOne', 'hasMany', 'hasOneThrough', 'hasManyThrough'];
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          type = _ref[_i];
          this.setRelationsType(type);
        }
        return this;
      },
      setRelationsType: function(type) {
        var param, params, _i, _len;
        if (params = this.relations[type]) {
          for (_i = 0, _len = params.length; _i < _len; _i++) {
            param = params[_i];
            this['set' + type.capitalize()](param);
          }
        }
        return this;
      },
      setBelongsTo: function(_arg) {
        var key, model, name, through;
        key = _arg.key, model = _arg.model, name = _arg.name, through = _arg.through;
        this.getter(name, (function(_this) {
          return function() {
            return model.find(_this[key]);
          };
        })(this));
        return this;
      },
      setHasOne: function(_arg) {
        var key, model, name, through;
        key = _arg.key, model = _arg.model, name = _arg.name, through = _arg.through;
        this.getter(name, (function(_this) {
          return function() {
            ...
            delete _this[name];
            (filters = {})[key] = _this.id;
            collection = model.where(filters);
            _this.getter(name, function() {
      ...
      setHasMany: function(_arg) {
        ...
        this.getter(name, (function(_this) {
          return function() {
            ...
            delete _this[name];
            (filters = {})[key] = _this.id;
            return _this[name] = model.where(filters);
          };
        })(this));
        return this;
      },
      setHasOneThrough: function(_arg) {
        var key, model, name, through;
        key = _arg.key, model = _arg.model, name = _arg.name, through = _arg.through;
        this.getter(name, (function(_this) {
          return function() {
      ...
      setHasManyThrough: function(_arg) {
        var key, model, name, through;
        key = _arg.key, model = _arg.model, name = _arg.name, through = _arg.through;
        this.getter(name, (function(_this) {
          return function() {
            delete _this[name];
            _this[name] = model.where({
              correct: function() {
                var _i, _len, _ref;
                _ref = this[through];
                for (_i = 0, _len = _ref.length; _i < _len; _i++) {
            ...
            _this[name].subscribeTo(_this[through], 'update.length.add', function(model) {
              return this.add(model[key]);
            });
            _this[name].subscribeTo(_this[through], 'update.length.remove', function(model) {
      ...
      view: function(name) {
        var view;
        if ((view = this.views[name]) == null) {
          if ((view = this.prototype.views[name]) != null) {
            view = this.views[name] = view.clone({
              model: this
            });
          } else {
            console.error('View "%s" of model "%s" does not exist', name, this._name);
          }
        }
        return view;
      },
      show: function(name, insertTo) {
        var view;
        if ((view = this.view(name)) != null) {
          return view.show(insertTo);
        } else {
          return null;
        }
      },
      hide: function(name, delay) {
        var view;
        if ((view = this.view(name)) != null) {
          return view.hide(delay);
        } else {
          return null;
        }
      },
      validations: {
        presence: function(value, filter) {
          if (filter) {
            return value != null;
          } else {
            return value == null;
        ...
        inclusion: function(value, filter) {
          return (value == null) || __indexOf.call(filter, value) >= 0;
        },
        exclusion: function(value, filter) {
          return (value == null) || __indexOf.call(filter, value) < 0;
        },
        length: function(value, filter) {
          var _ref;
          if (value == null) {
            return true;
          } else {
          ...
          if ((filter["in"] != null) && (_ref = value.length, __indexOf.call(filter["in"], _ref) < 0)) {
            return false;
          }
          if ((filter.min != null) && value.length < filter.min) {
            return false;
          }
          if ((filter.max != null) && value.length > filter.max) {
            return false;
          }
          if ((filter.is != null) && value.length !== filter.is) {
            return false;
          }
          return true;
        },
        format: function(value, filter) {
          if (value == null) {
            return true;
          }
          if (filter instanceof RegExp && filter.test(value)) {
            return true;
          }
          if (filter === 'boolean' && /^true|false$/.test(value)) {
            return true;
          }
          if (filter === 'number' && /^[0-9]+$/.test(value)) {
            return true;
          }
          if (filter === 'letters' && /^[A-zА-я]+$/.test(value)) {
            return true;
          }
          if (filter === 'email' && /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,4})+$/.test(value)) {
            return true;
          }
          return false;
        }
      ...
      isValidAttributeValue: function(name, value) {
        var filter, notice, params, tester, type, validation, _ref, _ref1;
        _ref = this.validations;
        for (validation in _ref) {
          tester = _ref[validation];
          if ((filter = (_ref1 = this.prototype.attributes[name]) != null ? _ref1[validation] : void 0) != null) {
            if (!tester.call(this, value, filter)) {
              console.warn('Attribute %s of model %O has not validate %s', name, this, validation);
              if (notice = this.prototype.attributes[name].notice) {
                for (type in notice) {
                  params = notice[type];
                  if (this.Notice[type] != null) {
                    this.Notice[type](params);
                  } else {
                    console.warn('Unknown Notice type "%s"', type);
                  }
                }
              }
              return false;
...
(function() {
  ...
  Nali.extend({
    Collection: {
      ...
      length: 0,
      cloning: function() {
        this.subscribeTo(this.Model, "create." + (this.model._name.lower()), this.onModelCreated);
        this.subscribeTo(this.Model, "update." + (this.model._name.lower()), this.onModelUpdated);
        this.subscribeTo(this.Model, "destroy." + (this.model._name.lower()), this.onModelDestroyed);
        this.adaptations = {
          apply: [],
          cancel: []
        };
        this.ordering = {};
        this.adaptCollection();
        this.refilter();
        return this;
      },
      refilter: function() {
        this.model.each((function(_this) {
          return function(model) {
            var isCorrect;
            if (isCorrect = model.isCorrect(_this.filters) && !(__indexOf.call(_this, model) >= 0)) {
              return _this.add(model);
            } else if (!isCorrect && __indexOf.call(_this, model) >= 0) {
      ...
      onModelCreated: function(model) {
        if (!this.freezed && model.isCorrect(this.filters)) {
          this.add(model);
        }
        return this;
      },
      onModelUpdated: function(model) {
        if (model.written()) {
          if (__indexOf.call(this, model) >= 0) {
            if (this.freezed || model.isCorrect(this.filters)) {
              this.reorder();
          ...
          } else if (!this.freezed && model.isCorrect(this.filters)) {
            this.add(model);
          }
        }
        return this;
      ...
      onModelDestroyed: function(model) {
        if (__indexOf.call(this, model) >= 0 && !this.freezed) {
          this.remove(model);
        }
        return this;
      },
      adaptCollection: function() {
        var method, name, _ref;
        _ref = this.model;
        for (name in _ref) {
          method = _ref[name];
          if (/^_\w+$/.test(name) && typeof method === 'function') {
            (function(_this) {
              return (function(name, method) {
                return _this[name = name.slice(1)] = function() {
                  var args;
                  args = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
      ...
      adaptModel: function(model, type) {
        var adaptation, _i, _len, _ref;
        if (type == null) {
          type = 'apply';
        }
        _ref = this.adaptations[type];
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          adaptation = _ref[_i];
          adaptation.call(this, model);
        }
        return this;
      },
      adaptation: function(apply, cancel) {
        this.each(function(model) {
          return apply.call(this, model);
        });
        this.adaptations.apply.push(apply);
        if (cancel) {
          this.adaptations.cancel.unshift(cancel);
        }
        return this;
      },
      add: function() {
        var model, models, _i, _len, _ref, _ref1;
        models = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
        _ref1 = (_ref = []).concat.apply(_ref, models);
        for (_i = 0, _len = _ref1.length; _i < _len; _i++) {
          model = _ref1[_i];
          Array.prototype.push.call(this, model);
          this.adaptModel(model);
          this.reorder();
          this.trigger('update.length.add', model);
          this.trigger('update.length', 'add', model);
        }
        return this;
      },
      remove: function(model) {
        this.adaptModel(model, 'cancel');
        Array.prototype.splice.call(this, this.indexOf(model), 1);
        this.unsubscribeFrom(model);
        this.reorder();
        this.trigger('update.length.remove', model);
        this.trigger('update.length', 'remove', model);
        return this;
      },
      removeAll: function() {
      ...
      each: function(callback) {
        ...
        for (index = _i = 0, _len = this.length; _i < _len; index = ++_i) {
          model = this[index];
          callback.call(this, model, index);
        }
        return this;
      },
      pluck: function(property) {
        ...
        for (_i = 0, _len = this.length; _i < _len; _i++) {
          model = this[_i];
          _results.push(model[property]);
        }
        return _results;
      },
      indexOf: function(model) {
        return Array.prototype.indexOf.call(this, model);
      },
      ...
      reorder: function() {
        if (this.ordering.by != null) {
          if (this.ordering.timer != null) {
            clearTimeout(this.ordering.timer);
          }
          this.ordering.timer = setTimeout((function(_this) {
            return function() {
              if (typeof _this.ordering.by === 'function') {
                _this.sort(_this.ordering.by);
              } else {
                _this.sort(function(one, two) {
                  one = one[_this.ordering.by];
                  two = two[_this.ordering.by];
                  if (_this.ordering.as === 'number') {
                    one = +one;
                    two = +two;
                  }
                  if (_this.ordering.as === 'string') {
                    one = '' + one;
                    two = '' + two;
                  }
                  return (one > two ? 1 : one < two ? -1 : 0) * (_this.ordering.desc ? -1 : 1);
                });
              }
              _this.orderViews();
              return delete _this.ordering.timer;
            };
          })(this), 5);
        }
        return this;
      ...
      orderViews: function() {
        ...
        if (this.inside) {
          children = Array.prototype.slice.call(this.inside.children);
          children.sort((function(_this) {
            return function(one, two) {
              return _this.indexOf(one.view.model) - _this.indexOf(two.view.model);
            };
          })(this));
          for (_i = 0, _len = children.length; _i < _len; _i++) {
            child = children[_i];
            this.inside.appendChild(child);
          }
        }
        return this;
      },
      show: function(viewName, insertTo, isRelation) {
        if (isRelation == null) {
          isRelation = false;
        }
        this.adaptation(function(model) {
          var view;
          view = model.view(viewName);
          if (isRelation) {
            view.subscribeTo(this, 'reset', view.hide);
          } else if (!this.visible) {
            this.visible = true;
            this.prepareViewToShow(view);
            this.hideVisibleViews();
          } else {
            this.prototype.visibleViews.push(view);
          }
          view.show(insertTo);
          return this.inside != null ? this.inside : this.inside = view.element[0].parentNode;
        }, function(model) {
          return model.hide(viewName);
        });
        return this;
      },
      hide: function(viewName, delay) {
        var model, _i, _len;
        for (_i = 0, _len = this.length; _i < _len; _i++) {
          model = this[_i];
          model.hide(viewName, delay);
        }
        return this;
      },
      prepareViewToShow: function(view) {
        var layout, _ref;
        if (__indexOf.call(this.prototype.toShowViews, view) < 0) {
          this.prototype.toShowViews.push(view);
          if ((_ref = (layout = view.layout())) != null ? typeof _ref.childOf === "function" ? _ref.childOf('View') : void 0 : void 0) {
            this.prepareViewToShow(layout);
          }
        }
        return this;
      },
      hideVisibleViews: function() {
        var view, _i, _len, _ref;
        _ref = this.prototype.visibleViews;
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          view = _ref[_i];
          if (!(__indexOf.call(this.prototype.toShowViews, view) >= 0)) {
            view.hide();
        ...
        this.prototype.visibleViews = this.prototype.toShowViews;
        this.prototype.toShowViews = [];
        return this;
      },
      first: function() {
        return this[0];
    ...
    Controller: {
      extension: function() {
        if (this._name !== 'Controller') {
          this.prepareActions();
          this.modelName = this._name.replace(/s$/, '');
        }
        return this;
      },
      "new": function(collection, filters, params) {
        return this.clone({
      ...
      prepareActions: function() {
        var action, filter, filters, name, params, _i, _len, _ref, _ref1, _ref2;
        this._actions = {};
        _ref = this.actions;
        for (name in _ref) {
          action = _ref[name];
          if (!(!(name === 'default' || name === 'before' || name === 'after'))) {
            continue;
          }
          _ref1 = name.split('/'), name = _ref1[0], filters = 2 <= _ref1.length ? __slice.call(_ref1, 1) : [];
          params = [];
          _ref2 = filters.slice(0);
          for (_i = 0, _len = _ref2.length; _i < _len; _i++) {
            filter = _ref2[_i];
            if (!(/^:/.test(filter))) {
              continue;
      ...
      prepareBefores: function() {
        var actions, list, name, _ref;
        if (((_ref = this.actions) != null ? _ref.before : void 0) != null) {
          list = this.analizeFilters('before');
          for (name in list) {
            actions = list[name];
            this._actions[name].methods = actions.concat(this._actions[name].methods);
          }
        }
        return this;
      },
      prepareAfters: function() {
        var actions, list, name, _ref;
        if (((_ref = this.actions) != null ? _ref.after : void 0) != null) {
          list = this.analizeFilters('after');
          for (name in list) {
            actions = list[name];
            this._actions[name].methods = this._actions[name].methods.concat(actions);
          }
        }
        return this;
      },
      analizeFilters: function(type) {
        var action, invert, list, name, names, _ref, _ref1;
        list = {};
        _ref = this.actions[type];
        for (names in _ref) {
          action = _ref[names];
          _ref1 = (function() {
            switch (false) {
              case !/^!\s*/.test(names):
                return [true, names.replace(/^!\s*/, '').split(/\s*,\s*/)];
              case names !== '*':
                return [true, []];
              default:
                return [false, names.split(/\s*,\s*/)];
            }
          })(), invert = _ref1[0], names = _ref1[1];
          for (name in this._actions) {
            if ((invert && !(__indexOf.call(names, name) >= 0)) || (!invert && __indexOf.call(names, name) >= 0)) {
              (list[name] != null ? list[name] : list[name] = []).push(action);
            }
          }
        }
        return list;
      },
      run: function(action, filters, params) {
        var collection;
        collection = this.Model.extensions[this.modelName].where(filters);
        this["new"](collection, filters, params).runAction(action);
        return this;
      },
      runAction: function(name) {
        var method, _i, _len, _ref;
        _ref = this._actions[name].methods;
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          method = _ref[_i];
          if (!this.stopped) {
            method.call(this);
      ...
      redirect: function() {
        var args, _ref;
        args = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
        (_ref = this.Router).redirect.apply(_ref, args);
        this.stop();
        return this;
      }
    }
  ...
  Nali.extend({
    Cookie: {
      set: function(name, value, options) {
        ...
        if ((options.live != null) && typeof options.live === 'number') {
          date = new Date;
        ...
        if (options.domain != null) {
          set += '; domain=' + escape(options.domain);
        }
        set += '; path=' + (options.path != null ? escape(options.path) : '/');
...
(function() {
  Nali.Model.extend({
    Notice: {
      initialize: function() {
        this.prototype.prototype.expand({
          Notice: this
        });
        ...
        for (name in this.views) {
          _results.push((function(_this) {
            return function(name) {
              return _this[name] = function(params) {
                return _this["new"](_this.prepare(params)).show(name);
              };
            };
          })(this)(name));
        }
...
(function() {
  Nali.extend({
    AudioNotice: {
      initialize: function() {
        return this.supported = typeof Audio === 'function' ? true : false;
      },
      list: [
        {
          name: 'Без звука',
          ...
          name: 'Капля воды 1',
          file: 'water_droplet_2'
        }, {
          name: 'Капля воды 2',
          file: 'water_droplet'
        }, {
          name: 'Капля воды 3',
          file: 'water_droplet_3'
        }, {
          name: 'Пивная пробка',
          file: 'beer_can_opening'
        }, {
          name: 'Колокольчик',
          file: 'bell_ring'
        }, {
          ...
          name: 'Фотозатвор 1',
          file: 'camera_flashing'
        }, {
          name: 'Фотозатвор 2',
          file: 'camera_flashing_2'
        }, {
          name: 'CD-лоток',
          file: 'cd_tray'
        }, {
          ...
          name: 'Клавиатурный лоток',
          file: 'keyboard_desk'
        }, {
          name: 'Лампочка разбилась',
          file: 'light_bulb_breaking'
        }, {
          name: 'Лист металла 1',
          file: 'metal_plate'
        }, {
      ...
      play: function(index) {
        var sound;
        sound = this.list[+index];
        if (!this.supported) {
          return this.Notice.error('Извините, но ваш браузер не поддерживает воспроизведение звука');
        } else if (sound.file != null) {
          if (sound.player == null) {
            sound.player = new Audio;
            sound.player.src = '/sounds/' + sound.file + this.ext();
            sound.player.load();
          }
    ...
    Homes: {
      actions: {
        ...
        index: function() {
          ...
          return this.collection.freeze().add(this.Model.Home["new"]());
        }
      }
    }
  });
  ...
  Nali.Controller.extend({
    Users: {
      actions: {
        ...
        after: {
          '! auth': function() {
            return this.collection.freeze().add(this.Application.user);
          }
        },
        index: function() {},
        person: function() {},
        ...
        'auth/:token': function() {
          if (this.params.token != null) {
            this.Application.auth(this.params.token, 'user');
            return this.stop();
          } else {
            return this.redirect('home');
          }
...
(function() {
  Nali.Model.extend({
    Contact: {
      ...
      onDestroy: function() {
        if (!this.destroyInitiator) {
          this.Notice.info("" + this.contact.name + " больше не хочет общаться с Вами, диалог удален");
        }
        if (this.active) {
      ...
      fresh: function(params) {
        this.Notice.info('Вам подобран новый собеседник');
        return this.toDialog();
      },
      toDialog: function() {
        this.redirect('dialog/' + this.dialog_id);
        return this;
      },
      activate: function() {
        this.upgrade({
      ...
      "delete": function() {
        this.destroyInitiator = true;
        this.contact.destroyInitiator = true;
        this.dialog.destroy();
    ...
    Dialog: {
      ...
      beforeShow: {
        index: function() {
          ...
          this.Application.activeDialog = this;
          return this.contact().activate().viewIndex().activeModeOn();
        }
      },
      afterHide: {
        index: function() {
          var _ref;
          this.Application.activeDialog = null;
          return (_ref = this.contact()) != null ? _ref.deactivate().viewIndex().activeModeOff() : void 0;
        }
      },
      contact: function() {
        ...
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          contact = _ref[_i];
          if (contact.user === this.Application.user) {
            return contact;
          }
        }
        return null;
      ...
      writes: function() {
        return this.viewIndex().showWrites();
      },
      sendPhotos: function(photosId) {
        return this.query('dialogs.send_photos', {
          dialog_id: this.id,
          photos_id: photosId
        });
      },
  ...
  Nali.Model.extend({
    Home: {
      onUpdateGender: function() {
        if ((this.color != null) && (this.gender != null)) {
          return this.query('users.create', {
            gender: this.gender,
            color: this.color
          }, (function(_this) {
            return function(token) {
              return _this.redirect('user/auth/' + token);
            };
          })(this), (function(_this) {
            return function() {
...
(function() {
  ...
  Nali.Model.extend({
    Message: {
      ...
      createMessage: function(_arg) {
        var photosId;
        photosId = _arg.photosId;
        if (this.text) {
          this.text = this.text.toString().trim().replace(/[\r|\n]+/g, '\n').replace(/\ +/g, ' ');
        }
        this.query('messages.create', {
...
(function() {
  Nali.Model.Notice.expand({
    newmsg: function() {
      if (this.AudioNotice.supported) {
        return this.AudioNotice.play(this.Application.user.sound);
      }
    }
  });
...
(function() {
  Nali.Model.extend({
    Photo: {
      ...
      uploadPhoto: function(image, progressCallback) {
        this.query('photos.upload_photo', {
          image: image
        }, progressCallback);
        return this;
      },
      uploadAvatar: function(image, success) {
        this.query('photos.upload_avatar', {
          image: image
        }, success);
        return this;
      },
      url: function(width, height, limit) {
        var h, m, w, _ref;
        if (limit == null) {
          limit = true;
        }
        _ref = limit ? ['limit', Math.floor(width), Math.floor(height)] : ['fill,g_faces', Math.ceil(width), Math.ceil(height)], m = _ref[0], w = _ref[1], h = _ref[2];
        return 'http://res.cloudinary.com/isite-im/image/upload/c_' + m + ',w_' + w + ',h_' + h + '/' + this.secret + '.jpg';
      },
      _selectModeOn: function() {
        return this.viewPreview().selectModeOn();
      },
      _selectModeOff: function() {
        this.update({
          selected: false
        });
        return this.viewPreview().selectModeOff();
      },
      _avatarModeOn: function() {
        return this.viewPreview().avatarModeOn();
      },
      _avatarModeOff: function() {
        return this.viewPreview().avatarModeOff();
      },
      toggleSelected: function() {
        return this.update({
          selected: this.selected ? false : true
        });
      }
    }
  });
...
(function() {
  ...
  Nali.Model.extend({
    User: {
      ...
      attributes: {
        name: {
          length: {
            "in": (function() {
              _results = [];
              for (_i = 3; _i <= 30; _i++){ _results.push(_i); }
              return _results;
            }).apply(this)
          },
          notice: {
        ...
        sound: {
          ...
          inclusion: (function() {
            _results1 = [];
            for (_j = 0; _j <= 25; _j++){ _results1.push(_j); }
            return _results1;
          }).apply(this)
        },
        avatar: null
      ...
      cloning: function() {
        return this.getter('avatarPath', function() {
          return 'http://res.cloudinary.com/isite-im/image/upload/' + this.avatar + '.jpg';
        });
      },
      beforeShow: {
        photos: function() {
          this.photo = this.Photo["new"]({
            user_id: this.id
          });
          this.photos.order({
            by: 'created',
          ...
          return this.selectedPhotos != null ? this.selectedPhotos : this.selectedPhotos = this.photos.where({
            selected: true
          });
        },
        "interface": function() {
          return this.contacts.order({
            by: function(one, two) {
              switch (false) {
                case !(one.counter > two.counter):
                  return -1;
                case !(one.counter < two.counter):
                  return 1;
                case !(one.contact.online && !two.contact.online):
                  return -1;
                case !(!one.contact.online && two.contact.online):
                  return 1;
                case !(one.contact.name < two.contact.name):
                  return -1;
                case !(one.contact.name > two.contact.name):
                  return 1;
                case !(one.contact.color < two.contact.color):
                  return -1;
                case !(one.contact.color > two.contact.color):
                  return 1;
                default:
      ...
      activateSearch: function() {
        ...
        return this.query('users.search');
      },
      ...
      toggleDialogs: function() {
        if (this._('.contacts').hasClass('show_contacts')) {
          return this.hideDialogs();
        } else {
          return this.showDialogs();
        }
      },
      showDialogs: function() {
        this._('.toolbar a.dialogs').addClass('button_hover');
        this._('.contacts').addClass('show_contacts');
        return this._(document).on('click.dialogs', (function(_this) {
          return function(event) {
            if (!_this._(event.target).closest('a.dialogs, div.contacts').length) {
              return _this.hideDialogs();
            }
          };
        })(this));
      },
      hideDialogs: function() {
        this._('.toolbar a.dialogs').removeClass('button_hover');
        this._('.contacts').removeClass('show_contacts');
        return this._(document).off('click.dialogs');
      },
      emailSend: function(_arg) {
        var email;
        email = _arg.email;
        if (/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,4})+$/.test(email)) {
          return this.query('users.to_email', {
            email: email
          }, (function(_this) {
            return function() {
              _this.hideEmail();
              return _this.Notice.info('Кнопка автологина отправлена на ' + email);
            };
          })(this));
        } else {
          return this.Notice.warning('Введите правильный e-mail');
        }
      },
      logout: function() {
      ...
      showSlides: function(slides, start) {
        return this.viewSlider().setSlides(slides, start).show();
      },
      sendSelectedPhotos: function() {
        var dialog, _ref;
        if (!(dialog = this.Application.activeDialog)) {
          return this.Notice.info('Откройте диалог с пользователем, которому хотите отправить фотографии');
        }
        if (!((_ref = this.selectedPhotos) != null ? _ref.length : void 0)) {
          return this.Notice.info('Выберите фотографии для отправки');
        }
        if (this.selectedPhotos.length > 10) {
          return this.Notice.info('Можно отправлять не более 10-ти фотографий сразу');
        }
        dialog.message.createMessage({
          photosId: this.selectedPhotos.pluck('id')
        });
      ...
      deleteSelectedPhotos: function() {
        var _ref;
        if ((_ref = this.selectedPhotos) != null ? _ref.length : void 0) {
          return this.showDeletePhotos();
        } else {
          return this.Notice.info('Выберите фото для удаления');
        }
      },
      deletePhotos: function() {
        var photo, _k, _len, _ref, _ref1;
        if ((_ref = this.selectedPhotos) != null ? _ref.length : void 0) {
          _ref1 = this.selectedPhotos;
          for (_k = 0, _len = _ref1.length; _k < _len; _k++) {
            photo = _ref1[_k];
            photo.destroy();
          }
        }
        this.viewPhotos().selectModeOff();
        return this.hideDeletePhotos();
      },
      changeAvatar: function() {
        return this.showPhotos().avatarModeOn().cancelIsClose = true;
      },
      deleteAvatar: function() {
        return this.query('users.delete_avatar');
      }
    }
  });
...
(function() {
  Nali.View.extend({
    ContactDelete: {
      ...
      events: ['hide on click at    .wrapper', 'notHide on click at .dialog'],
      notHide: function(event) {
...
(function() {
  Nali.View.extend({
    ContactIndex: {
      ...
      helpers: {
        online: function() {
          if (this.getOf(this.my.contact, 'online')) {
            return this.getOf(this.my.contact, 'color') + '_contact';
          } else {
            return '';
          }
        }
      ...
      activeModeOn: function() {
        return this.element.addClass('active');
      },
      activeModeOff: function() {
...
(function() {
  Nali.View.extend({
    DialogIndex: {
      layout: function() {
        return this.Application.user.viewInterface();
      },
      onShow: function() {
        this.Application.setTitle('Диалог');
        this.messagesBox = this.element.find('.messages');
        this.messagesBox.niceScroll({
          mousescrollstep: 80
        });
        return this.messagesBox.on('scroll', (function(_this) {
          return function(event) {
            if (event.currentTarget.scrollTop === 0) {
              return _this.my.loadHistory();
            }
          };
        })(this));
      },
      onHide: function() {
        this.messagesBox.getNiceScroll().remove();
        return this.messagesBox.off();
      },
      scrollTo: function(message) {
        this.hideWrites();
        return this.messagesBox[0].scrollTop += message.element.height();
      },
      showWrites: function() {
        if (this.writesTimer != null) {
          clearTimeout(this.writesTimer);
        }
        if (this.writesBox == null) {
          this.writesBox = this.element.find('.writes');
        }
        this.writesBox.addClass('show_writes');
        this.writesTimer = setTimeout(((function(_this) {
          return function() {
            return _this.hideWrites();
          };
        })(this)), 3500);
        return this;
      },
      hideWrites: function() {
        ...
        if ((_ref = this.writesBox) != null) {
          _ref.removeClass('show_writes');
        }
        this.writesTimer = null;
        return this;
      }
    }
  });
...
(function() {
  Nali.View.extend({
    HomeIndex: {
      onShow: function() {
        var colors;
        this.Application.setTitle('Сервис анонимного общения');
        colors = this.element.find('.colors').addClass('show');
        return setTimeout((function(_this) {
          return function() {
            var text;
            colors.addClass('shown').removeClass('show');
            return (text = _this.element.find('.logo div:nth-child(2)')).animate({
              opacity: 0
            }, 300, function() {
...
(function() {
  Nali.View.extend({
    MessageBuild: {
      events: ['toggleEmoticons            on click    at .emoticons', 'pasteEmoticon              on click    at .emoticons_list i', 'formSubmit                 on submit   at form', 'writesMessage, sendByEnter on keypress at textarea', 'addNewLine                 on keydown  at textarea'],
      onShow: function() {
        this.form = this.element.find('form');
        this.textarea = this.form.find('.message textarea');
        this.emoticons = this.element.find('.emoticons_list');
        return this.textarea.autosize();
      },
      onHide: function() {
        var _ref;
        if ((_ref = this.emoticons.getNiceScroll()) != null) {
          _ref.remove();
        }
      ...
      formSubmit: function(event) {
        ...
        if (this.emoticons.hasClass('emoticons_show')) {
          this.hideEmoticons();
        }
        return this.textarea.trigger('autosize.resize');
      },
      toggleEmoticons: function(event) {
        if (this.emoticons.hasClass('emoticons_show')) {
          return this.hideEmoticons();
        } else {
          return this.showEmoticons();
        }
      },
      showEmoticons: function() {
        this.emoticons.addClass('emoticons_show');
        setTimeout(((function(_this) {
          return function() {
            return _this.emoticons.niceScroll();
          };
        })(this)), 200);
        return this;
      },
      hideEmoticons: function() {
        this.emoticons.removeClass('emoticons_show');
        this.emoticons.getNiceScroll().remove();
        return this;
      },
      pasteEmoticon: function(event) {
        var emoticon;
        emoticon = String.fromCharCode(58880 + parseInt(event.target.className.replace('emoticon-', '')));
        return this.textarea.insertAtCaret(emoticon).trigger('change').trigger('autosize.resize');
      },
      ...
      addNewLine: function(event) {
        if (event.which === 13 && (event.ctrlKey || event.metaKey)) {
          return this.textarea.insertAtCaret(String.fromCharCode(10));
        }
      },
      writesMessage: function(event) {
        var newLastTime;
        if (event.which !== 13 && (newLastTime = new Date().getTime()) - (this.lastWriteTime != null ? this.lastWriteTime : this.lastWriteTime = 0) > 3000) {
          this.lastWriteTime = newLastTime;
          return this.query('dialogs.writes');
        }
      }
    }
  });
...
(function() {
  ...
  Nali.View.extend({
    MessageIndex: {
      helpers: {
        owner: function() {
          if (this.my.user === this.Application.user) {
            return 'Я';
          } else {
            return this.getOf(this.my.user, 'name');
          }
        },
        text: function() {
          var code, message, sym, text, _i, _j, _len, _results;
          text = '<p>';
          if (this.my.text != null) {
            message = ("" + this.my.text).replace(/&/g, '&amp;').replace(/"/g, '&quot;').replace(/'/g, '&#039;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
            for (_i = 0, _len = message.length; _i < _len; _i++) {
              sym = message[_i];
              code = sym.charCodeAt(0);
              if (__indexOf.call((function() {
                _results = [];
                for (_j = 58881; _j <= 58978; _j++){ _results.push(_j); }
                return _results;
              }).apply(this), code) >= 0) {
                text += '<i>' + sym + '</i> ';
              } else if (code === 10) {
      ...
      onShow: function() {
        this.resizingPhotos();
        this.subscribeTo(this.my.messagephotos, 'update.length', this.resizingPhotos);
        return this.my.dialog.viewIndex().scrollTo(this);
      },
      resizingPhotos: function() {
        var count, first, height, i, index, last, lastWidth, lines, matrix, messagephoto, start, startWidth, _i, _j, _k, _len, _ref, _results;
        if (count = this.my.messagephotos.length) {
          matrix = [];
          ...
          for (i = _i = 0; 0 <= first ? _i < first : _i > first; i = 0 <= first ? ++_i : --_i) {
            matrix.push(startWidth);
          }
          for (i = _j = 0; 0 <= last ? _j < last : _j > last; i = 0 <= last ? ++_j : --_j) {
            matrix.push(lastWidth);
          }
          _ref = this.my.messagephotos;
          _results = [];
          for (index = _k = 0, _len = _ref.length; _k < _len; index = ++_k) {
            messagephoto = _ref[index];
            _results.push(messagephoto.viewPreview().resize(matrix[index], height));
          }
          return _results;
        }
      }
...
(function() {
  Nali.View.extend({
    MessagephotoPreview: {
      ...
      onShow: function() {
        return this.preview != null ? this.preview : this.preview = this.element.find('.preview');
      },
      open: function(event) {
        event.stopPropagation();
        return this.Application.user.showSlides(this.my.message.photos, this.my.photo);
      },
      resize: function(w, h) {
        this.element[0].style.width = w;
        this.preview[0].style.paddingTop = h;
        return this.preview.css({
          backgroundImage: 'url(' + this.my.photo.url(this.element.width(), this.element.height(), false) + ')'
        });
      }
    }
  });
...
(function() {
  Nali.View.NoticeWarning.expand({
    hideDelay: 200,
    insertTo: function() {
      return '.UserInterface';
    },
...
(function() {
  Nali.View.extend({
    PhotoAvatar: {
      ...
      onShow: function() {
        var h, photoBox, w;
        photoBox = this.element.find('.photo');
        w = photoBox.width();
        h = photoBox.height();
        return this.photo = this._("<img src=\"" + (this.my.url(w, h)) + "\" crossorigin=\"anonymous\" />").appendTo(photoBox).Jcrop({
          aspectRatio: 1,
          minSize: [64, 64],
          onSelect: (function(_this) {
            return function(coord) {
              return _this.onSelect(coord);
            };
          })(this)
        });
      },
      onHide: function() {
        this.photo.data('Jcrop').destroy();
        this.photo.remove();
        return delete this.coord;
      },
      ...
      crop: function() {
        var canvas, context;
        if (!this.coord) {
          return this.Notice.info('Выделите область изображения для создания аватара');
        }
        canvas = document.createElement('canvas');
        canvas.width = 64;
        canvas.height = 64;
        context = canvas.getContext('2d');
        context.drawImage(this.photo[0], this.coord.x, this.coord.y, this.coord.w, this.coord.h, 0, 0, 64, 64);
        return this.my.uploadAvatar(canvas.toDataURL('image/jpeg'), (function(_this) {
          return function() {
            _this.hide();
            return _this.Notice.info('Аватар сохранен');
          };
        })(this));
      }
    }
...
(function() {
  Nali.View.extend({
    PhotoBuild: {
      events: ['onDrop      on drop      at .dropBox', 'onDragEnter on dragenter at .dropBox', 'onDragOver  on dragover  at .dropBox', 'onDragLeave on dragleave at .dropBox', 'selectFile  on click     at .dropBox', 'onSelect    on change    at input[type=file]'],
      onShow: function() {
        this.prepareIndicator = this.element.find('.prepareBar .progress');
        this.uploadingIndicator = this.element.find('.uploadingBar .progress');
        this.dropBox = this.element.find('.dropBox');
        return this.selectField = this.element.find('input[type=file]');
      },
      onDragEnter: function(event) {
        event.stopPropagation();
        return this.dropBox.addClass('dragenter');
      },
      onDragLeave: function(event) {
        event.stopPropagation();
        return this.dropBox.removeClass('dragenter');
      },
      ...
      selectFile: function(event) {
        event.stopPropagation();
        return this.selectField.click();
      },
      onDrop: function(event) {
        event.preventDefault();
        event.stopPropagation();
        this.dropBox.removeClass('dragenter');
        return this.prepareImages(Array.prototype.slice.call(event.originalEvent.dataTransfer.files));
      },
      onSelect: function(event) {
        event.stopPropagation();
        return this.prepareImages(Array.prototype.slice.call(event.target.files));
      },
      checkImages: function(files) {
        var ext, file, _i, _len, _ref;
        for (_i = 0, _len = files.length; _i < _len; _i++) {
          file = files[_i];
          ext = file.name.split('.').reverse()[0].lower();
          if (!((ext === 'jpg' || ext === 'png' || ext === 'gif') && ((_ref = file.type) === 'image/jpeg' || _ref === 'image/png' || _ref === 'image/gif'))) {
            this.Notice.info('Загружать разрешено только изображения');
            return false;
          }
        }
        return files;
      ...
      prepareImages: function(files) {
        var images;
        if (files.length > 10) {
          this.Notice.info('Пожалуйста, не загружайте более 10-ти фотографий одновременно');
        } else if (images = this.checkImages(files)) {
          this.prepareStart(images);
        }
        return this;
      },
      prepareStart: function(images) {
        var image, reader, _i, _len;
        this.element.addClass('preparing');
        this.prepared = [];
        this.prepareCount = images.length;
        for (_i = 0, _len = images.length; _i < _len; _i++) {
          image = images[_i];
          reader = new FileReader();
          reader.onload = (function(_this) {
            return function(event) {
              return _this.resizeImage(event.target.result);
            };
          })(this);
      ...
      prepareProgress: function(dataUrl) {
        var width;
        this.prepared.push(dataUrl);
        width = this.prepared.length / this.prepareCount * 100 + '%';
        this.prepareIndicator[0].style.width = width;
        if (this.prepared.length === this.prepareCount) {
          this.prepareEnd();
        }
        return this;
      },
      prepareEnd: function() {
        this.prepareIndicator[0].style.width = '0%';
        this.element.removeClass('preparing');
        this.uploadStart();
        return this;
      },
      uploadStart: function() {
        ...
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          image = _ref[_i];
          this.my.uploadPhoto(image, (function(_this) {
            return function() {
              return _this.uploadProgress();
            };
          })(this));
        }
        return this;
      ...
      uploadProgress: function() {
        ...
        width = this.uploadedCount / this.prepared.length * 100 + '%';
        this.uploadingIndicator[0].style.width = width;
        if (this.prepared.length === this.uploadedCount) {
      ...
      uploadEnd: function() {
        this.prepared = [];
        this.uploadedCount = 0;
        this.uploadingIndicator[0].style.width = '0%';
        return this.element.removeClass('uploading');
      },
      getNewSize: function(width, height) {
        var max;
        max = 800;
        switch (false) {
          case !(height >= width && height > max):
            return [Math.round(max * width / height), max];
          case !(height <= width && width > max):
            return [max, Math.round(max * height / width)];
          default:
            return [width, height];
        }
      },
      resizeImage: function(dataUrl) {
        ...
        image.onload = (function(_this) {
          return function(event) {
            var canvas, context, height, img, width, _ref;
            img = event.target;
            _ref = _this.getNewSize(img.width, img.height), width = _ref[0], height = _ref[1];
            canvas = document.createElement('canvas');
            canvas.width = width;
            canvas.height = height;
            context = canvas.getContext('2d');
            context.drawImage(img, 0, 0, width, height);
            return _this.prepareProgress(canvas.toDataURL('image/jpeg'));
          };
        })(this);
  ...
  Nali.View.extend({
    PhotoPreview: {
      ...
      onShow: function() {
        ...
        return this.preview.css({
          backgroundImage: 'url(' + this.my.url(this.element.width(), this.element.height(), false) + ')'
        });
      },
      openOrMode: function(event) {
        event.stopPropagation();
        if (this.selectMode) {
          this.element.toggleClass('unselected');
          return this.my.toggleSelected();
        } else if (this.avatarMode) {
          this.my.user.viewPhotos().avatarModeOff();
          return this.my.showAvatar();
        } else {
          return this.my.user.showSlides(this.my.user.photos, this.my);
        }
      },
      selectModeOn: function() {
        if (this.visible) {
          this.selectMode = true;
          return this.element.addClass('unselected');
        }
      },
      selectModeOff: function() {
        if (this.visible) {
          this.selectMode = false;
          return this.element.removeClass('unselected');
        }
      },
      avatarModeOn: function() {
        if (this.visible) {
          this.avatarMode = true;
          return this.element.addClass('forAvatar');
        }
      },
      avatarModeOff: function() {
        if (this.visible) {
          this.avatarMode = false;
          return this.element.removeClass('forAvatar');
        }
      }
    }
  });
...
(function() {
  Nali.View.extend({
    UserAccount: {
      layout: function() {
        return this.my.viewSettings();
      },
      onShow: function() {
        this.Application.setTitle('Аккаунт');
        return this.layout().activeTab('.account');
      }
    }
  });
    ...
    UserBookmark: {
      ...
      helpers: {
        hotkeys: function() {
          ...
          switch (false) {
            case __indexOf.call(ua, 'mac') < 0:
              return 'Command/Cmd + D';
            case __indexOf.call(ua, 'konqueror') < 0:
              return 'Ctrl + B';
            default:
              return 'Ctrl + D';
          }
      ...
      onShow: function() {
        this.Application.setTitle('iSite.im');
        return this.Router.changeUrl('user/auth/' + this.Cookie.get('token'));
      },
      onHide: function() {
        this.Application.setTitle('Аккаунт');
        return this.Router.changeUrl('user/account');
      },
      notHide: function(event) {
        return event.stopPropagation();
      }
...
(function() {
  Nali.View.extend({
    UserColor: {
      ...
      onShow: function() {
        this.element.addClass('show');
        this._('a.colors').addClass('button_hover');
        return this._(document).on('click.colors', (function(_this) {
          return function(event) {
            if (!_this._(event.target).closest('a.colors, div.UserColor').length) {
              return _this.hide();
            }
          };
        })(this));
      ...
      onHide: function() {
        ...
        this._('a.colors').removeClass('button_hover');
        return this._(document).off('click.colors');
      }
    }
  });
...
(function() {
  Nali.View.extend({
    UserFace: {
      helpers: {
        face: function() {
          if (this.getMy('avatar') != null) {
            return '<span class="avatar" style="background-image: url(' + this.my.avatarPath + ')"></span>';
          } else {
            return '<i class="faceicon-' + this.getMy('gender') + this.getMy('image') + '"></i>';
          }
        }
      }
    }
...
(function() {
  ...
  Nali.View.extend({
    UserIndex: {
      layout: function() {
        return this.my.viewInterface();
      },
      helpers: {
        stylize: function() {
          if (this.getOf(this.my.contacts, 'length')) {
            this.Application.setTitle('Диалоги');
            return 'show_select';
          } else {
            this.Application.setTitle('Поиск');
            return 'show_search_' + (this.getMy('search') ? 'on' : 'off');
          }
        },
        ending: function() {
          var count, end;
          count = this.my.contacts.length + '';
          end = parseInt(count[count.length - 1]);
          if (__indexOf.call([2, 3, 4], end) >= 0) {
            return 'а';
          } else if (end > 4) {
            return 'ов';
          } else {
      ...
      onShow: function() {
        this._('a.face').addClass('button_hover');
        if (this.my.contacts.length === 0) {
          return this.my.activateSearch();
        }
      },
      onHide: function() {
        return this._('a.face').removeClass('button_hover');
      }
    }
  });
...
(function() {
  Nali.View.extend({
    UserInterface: {
      ...
      onShow: function() {
        this.element.find('.contactsWrapper').niceScroll({
          railalign: 'left'
        });
        this.subscribeTo(this.my, 'update.color', this.changeFavicon);
        return this.changeFavicon();
      },
      onHide: function() {
        return this.element.find('.contactsWrapper').getNiceScroll().remove();
      },
      changeFavicon: function() {
        return this._('#favicon').replaceWith('<link id="favicon" rel="shortcut icon" href="/images/favicons/' + this.my.color + '.ico" type="image/x-icon">');
      }
    }
  });
...
(function() {
  Nali.View.extend({
    UserPerson: {
      ...
      onShow: function() {
        this.Application.setTitle('Образ');
        return this.layout().activeTab('.person');
      }
    }
  });
...
(function() {
  Nali.View.extend({
    UserPhotos: {
      ...
      events: ['hide on click at    .wrapper', 'notHide on click at .photos'],
      helpers: {
        selectedCount: function() {
          var count;
          if (count = this.getOf(this.my.selectedPhotos, 'length')) {
            return 'Отмечено ' + count + ' фото';
          } else {
            return 'Нажмите на фото, чтобы отметить его';
          }
        }
      },
      onShow: function() {
        this._('a.photos').addClass('button_hover');
        if (this.photosBox == null) {
          this.photosBox = this.element.find('.previews');
        }
        return this.photosBox.niceScroll({
          cursorborder: '1px #333 solid'
        });
      },
      onHide: function() {
        this._('a.photos').removeClass('button_hover');
        this.cancelIsClose = false;
        this.selectModeOff();
        this.avatarModeOff();
        return this.photosBox.getNiceScroll().remove();
      },
      notHide: function(event) {
        return event.stopPropagation();
      },
      selectModeOn: function() {
        if (this.my.photos.length) {
          this.element.find('.photos').addClass('selectionMode');
          this.my.photos.selectModeOn();
        } else {
          this.Notice.info('Для начала загрузите фотографии');
        }
        return this;
      },
      selectModeOff: function() {
        this.element.find('.photos').removeClass('selectionMode');
        this.my.photos.selectModeOff();
        return this;
      },
      avatarModeOn: function() {
        if (this.my.photos.length) {
          this.element.find('.photos').addClass('avatarMode');
          this.my.photos.avatarModeOn();
        } else {
          this.Notice.info('Для начала загрузите фотографии');
        }
        return this;
      ...
      avatarModeOff: function() {
        if (this.cancelIsClose) {
          this.hide();
        } else {
          this.element.find('.photos').removeClass('avatarMode');
          this.my.photos.avatarModeOff();
        }
        return this;
      }
    }
  ...
  Nali.View.extend({
    UserSearch: {
      ...
      onShow: function() {
        this.Application.setTitle('Поиск');
        this.activateRadio();
        this.subscribeTo(this.my, 'update.how', this.activateRadio);
        this.subscribeTo(this.my, 'update.who', this.activateRadio);
        return this.layout().activeTab('.search');
      },
      activateRadio: function() {
        this.element.find('.radio_active').removeClass('radio_active');
        return this.element.find('input:radio:checked').parent().addClass('radio_active');
      }
    }
  });
...
(function() {
  Nali.View.extend({
    UserSettings: {
      ...
      onShow: function() {
        this._('a.settings').addClass('button_hover');
        return this["yield"].niceScroll({
          railoffset: {
            left: 14
          }
        });
      ...
      onHide: function() {
        this._('a.settings').removeClass('button_hover');
        return this["yield"].getNiceScroll().remove();
      },
      activeTab: function(selector) {
        var index;
        if (this.tabs == null) {
          this.tabs = this.element.find('.settingsBar > .tabs');
        }
        if (this.tabsCount == null) {
          this.tabsCount = this.tabs.children().length;
        }
        if (this.marker == null) {
          this.marker = this.element.find('.settingsBar > .marker').width(100 / this.tabsCount + '%');
        }
        index = this.tabs.find('.active').removeClass('active').end().find(selector).addClass('active').index();
        this.marker.css({
          marginLeft: index / this.tabsCount * 100 + '%'
        });
        return this["yield"][0].scrollTop = 0;
      }
    }
  });
  ...
  Nali.View.extend({
    UserSlider: {
      ...
      onShow: function() {
        ...
        this.slideW = this.slide.width();
        this.slideH = this.slide.height();
      ...
      setSlides: function(slides, current) {
        this.slides = slides;
      ...
      startSlider: function() {
        this.showSlide();
        this.preload(this.nextSlide());
        this.preload(this.prevSlide());
        return this;
      },
      showSlide: function() {
        return this.slide.html('').append(this._('<img src="' + this.current.url(this.slideW, this.slideH) + '" alt="" />'));
      },
      next: function(event) {
        event.stopPropagation();
        this.current = this.nextSlide();
        this.showSlide();
        return this.preload(this.nextSlide());
      },
      prev: function() {
        this.current = this.prevSlide();
        this.showSlide();
        return this.preload(this.prevSlide());
      },
      nextSlide: function() {
        var slide;
        if ((slide = this.slides[this.slides.indexOf(this.current) + 1]) != null) {
          return slide;
        } else {
      ...
      preload: function(slide) {
        var image;
        image = new Image;
        image.src = slide.url(this.slideW, this.slideH);
        return this;
      }
    }
  });
...
(function() {
  Nali.View.extend({
    UserSounds: {
      ...
      onShow: function() {
        this.Application.setTitle('Звуки');
        this.activateRadio();
        this.subscribeTo(this.my, 'update.sound', this.activateRadio);
        return this.layout().activeTab('.sounds');
      },
      activateRadio: function() {
        this.element.find('.checked').removeClass('checked');
        return this.element.find('input:radio:checked').parent().addClass('checked');
      }
    }
  });
...
(function() {
  Nali.Application.expand({
    ...
    auth: function(token, url) {
      return this.query('users.auth', {
        token: token
      }, (function(_this) {
        return function(id) {
          _this.user = _this.Model.User.find(id);
