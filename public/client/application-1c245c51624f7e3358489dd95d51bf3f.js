  ...
  window.Nali = {
    ...
    trigger: function() {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        item = _ref[_i];
        if (item[1] === event) {
          (_ref1 = item[2]).call.apply(_ref1, [item[0], this].concat(__slice.call(args)));
        }
      }
      return this;
    }
      ...
      await: function(callback) {
        ...
        wrapper = (function(_this) {
          return function() {
            var args;
            args = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
            _this.results.push(callback.apply(null, args));
            _this.awaits.splice(_this.awaits.indexOf(wrapper), 1);
            if (!_this.awaits.length) {
              return _this.defer(_this.results);
            }
          };
        })(this);
        this.awaits.push(wrapper);
        return wrapper;
      }
    }
  });
  ...
  Nali.extend({
    Connection: {
      ...
      notice: function(_arg) {
        var id, model, notice, params, _ref;
        model = _arg.model, notice = _arg.notice, params = _arg.params;
        if (model != null) {
    ...
    Router: {
        ...
        this.prototype.expand({
          redirect: (function(_this) {
            return function() {
              var args;
              args = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
              return _this.go.apply(_this, args);
            };
          })(this)
        });
        return this;
      ...
      start: function() {
        ...
        this._(window).on('popstate', (function(_this) {
          return function(event) {
            ...
            return _this.go(event.target.location.pathname);
          };
        })(this));
        return this;
      },
      ...
      go: function(url, options) {
        ...
        if (found = this.findRoute(this.prepare(url) || this.prepare(this.Application.defaultUrl))) {
          ...
          this.go(this.Application.notFoundUrl);
        } else {
    ...
    View: {
      ...
      insertTo: function() {
        ...
        if ((_ref = (layout = this.layout())) != null ? typeof _ref.childOf === "function" ? _ref.childOf('View') : void 0 : void 0) {
          return layout.show().element.find('.yield');
        } else {
      ...
      show: function(insertTo) {
        ...
        this.prepareElement().draw().bindEvents();
        if (!this.visible) {
          this.runModelCallback('beforeShow');
          this.runAssistants('show');
          this.subscribeTo(this.model, 'update', this.onSourceUpdated);
      ...
      showRelations: function() {
        var insertTo, name, relation, selector, view, _i, _len, _ref, _ref1;
        _ref = this.relationsMap;
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          _ref1 = _ref[_i], selector = _ref1.selector, name = _ref1.name, view = _ref1.view;
          if ((relation = this.model[name]) != null) {
            insertTo = this.element.find(selector);
            if (relation.childOf('Collection')) {
              relation.show(view, insertTo, true);
              relation.subscribeTo(this, 'hide', relation.reset);
            } else {
              view = relation.show(view, insertTo);
              view.subscribeTo(this, 'hide', view.hide);
            }
          } else {
            console.warn("Relation %s does not exist of model %O", name, this.model);
          }
        }
        return this;
      },
      ...
      runForm: function(event) {
        event.preventDefault();
        this.runUrl(event.currentTarget.getAttribute('action'), this.formToHash(event.currentTarget));
        return this;
      },
      runUrl: function(url, params) {
        var data, match, method, name, obj, specification, value, _i, _len, _ref, _ref1, _ref2;
        if (params == null) {
          params = {};
        }
        if (match = url.match(/^(@@?)(.+)/)) {
          _ref = match[2].split('?'), method = _ref[0], data = _ref[1];
          if (data) {
            _ref1 = data.split(/&|&amp;/);
            for (_i = 0, _len = _ref1.length; _i < _len; _i++) {
              specification = _ref1[_i];
              if (!(specification)) {
                continue;
              }
              _ref2 = specification.split('='), name = _ref2[0], value = _ref2[1];
              params[name] = value;
            }
          }
          obj = match[1].length === 1 ? this : this.model;
          if ((obj[method] != null) && typeof obj[method] === 'function') {
            obj[method](params);
          } else {
            console.warn("Method %s not exists", method);
          }
        } else {
          this.Router.go(url, params);
        }
        return this;
      },
      formToHash: function(form) {
        var element, key, keys, name, params, property, target, _i, _j, _len, _len1, _ref;
        params = {};
        _ref = form.elements;
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          element = _ref[_i];
          if (name = element.name || element.id) {
            property = (keys = name.match(/[^\[\]]+/g)).pop();
            target = params;
            for (_j = 0, _len1 = keys.length; _j < _len1; _j++) {
              key = keys[_j];
              target = target[key] instanceof Object ? target[key] : target[key] = {};
            }
      ...
      parseTemplate: function() {
        ...
        if (container = document.querySelector('#' + this._name.underscore())) {
          this.template = container.innerHTML.trim().replace(/\s+/g, ' ').replace(/({\s*\+.+?\s*})/g, ' <assist>$1</assist>').replace(/{\s*yield\s*}/g, '<div class="yield"></div>');
      ...
      parseRelations: function() {
        this.relationsMap = [];
        this.template = this.template.replace(/{\s*(\w+) of @(\w+)\s*}/g, (function(_this) {
          return function(match, view, relation) {
            var className;
            className = relation.capitalize() + view.capitalize() + 'Relation';
            _this.relationsMap.push({
              selector: '.' + className,
              name: relation,
              view: view
            });
      ...
      analizeChain: function(chain) {
        var property, segment, segments, source, _i, _len;
        ...
        if (!(property in source)) {
          console.warn("%s: chain \"%s\" is invalid, \"%s\" is not Object", this._name, chain, segment);
          return null;
        }
        return [source, property];
      }
  ...
  Nali.extend({
    Model: {
      ...
      notice: function(params) {
        this.noticesWait.push(params);
        this.runNotices();
        return this;
      },
      runNotices: function() {
        var index, item, model, _i, _len, _ref;
        _ref = this.noticesWait.slice(0);
        for (index = _i = 0, _len = _ref.length; _i < _len; index = ++_i) {
          item = _ref[index];
          if (model = this.extensions[item.model].find(item.id)) {
            model[item.notice](item.params);
            this.noticesWait.splice(this.noticesWait.indexOf(item), 1);
          }
        }
        return this;
      },
      ...
      sync: function(_arg) {
        ...
        if (model = this.extensions[_name].find(attributes.id)) {
          if (destroyed) {
            ...
            model.update(attributes, updated, created);
          }
        } else {
      ...
      update: function(attributes, updated, created) {
        ...
        if (!updated || updated > this.updated) {
          if (created) {
          ...
          for (name in attributes) {
            value = attributes[name];
            if (this.updateAttribute(name, value)) {
              changed.push(name);
            }
          }
          if (changed.length) {
      ...
      updateAttribute: function(name, value) {
        ...
        if (this.attributes[name] !== value && this.isValidAttributeValue(name, value)) {
      ...
      where: function(filters) {
        var attributes, collection, key, model, value, _i, _len, _ref, _ref1;
        collection = this.Collection["new"](this, filters);
        _ref = this.table;
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          model = _ref[_i];
        ...
        if (this.forced && !collection.length) {
          attributes = {};
          for (key in filters) {
            value = filters[key];
            if ((_ref1 = typeof value) === 'number' || _ref1 === 'string') {
              attributes[key] = value;
            }
          }
          collection.add(this["new"](attributes));
        }
        return collection;
      },
      all: function() {
      ...
      defaultAttributes: function() {
        ...
        for (name in _ref) {
          ...
          if (value instanceof Object) {
            attributes[name] = value["default"] != null ? value["default"] : null;
          } else {
            attributes[name] = value;
          }
        }
      ...
      isCorrect: function(filters) {
        ...
        for (name in filters) {
          filter = filters[name];
          if (!this.isCorrectAttribute(this.attributes[name], filter)) {
            return false;
          }
        }
        return true;
      ...
      setHasManyThrough: function(_arg) {
        ...
        this.getter(name, (function(_this) {
          return function() {
            delete _this[name];
            _this[name] = _this.Collection["new"](model, function() {
              var _i, _len, _ref;
              _ref = this[through];
              for (_i = 0, _len = _ref.length; _i < _len; _i++) {
                model = _ref[_i];
            ...
            _this[name].add(_this[through].pluck(key));
            _this[name].subscribeTo(_this[through], 'update.length.add', function(collection, model) {
              return this.add(model[key]);
            });
            _this[name].subscribeTo(_this[through], 'update.length.remove', function(collection, model) {
      ...
      view: function(name) {
        ...
        if ((view = this.views[name]) == null) {
          if ((view = this.prototype.views[name]) != null) {
            ...
            console.error("View %s of model %O does not exist", name, this);
          }
        }
        return view;
      },
      ...
      isValidAttributeValue: function(name, value) {
        var filter, message, tester, type, validation, _i, _len, _ref, _ref1, _ref2;
        ...
        for (validation in _ref) {
          ...
          if ((filter = (_ref1 = this.prototype.attributes[name]) != null ? _ref1[validation] : void 0) != null) {
            if (!tester.call(this, value, filter)) {
              ...
              _ref2 = ['info', 'warning', 'error'];
              for (_i = 0, _len = _ref2.length; _i < _len; _i++) {
                type = _ref2[_i];
                if ((message = this.prototype.attributes[name][type]) != null) {
                  this.Notice[type]({
                    message: message
                  });
                }
...
(function() {
  ...
  Nali.extend({
    Collection: {
      ...
      onModelCreated: function(extModel, model) {
        if (!this.freezed && model.isCorrect(this.filters)) {
          this.add(model);
        }
        return this;
      },
      onModelUpdated: function(extModel, model) {
        if (model.written()) {
      ...
      add: function() {
        ...
        for (_i = 0, _len = _ref1.length; _i < _len; _i++) {
          ...
          this.subscribeTo(model, 'destroy', this.onModelDestroyed);
          this.reorder();
  ...
  Nali.extend({
    Controller: {
      ...
      redirect: function() {
        var args, _ref;
        args = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
        (_ref = this.Router).go.apply(_ref, args);
        this.stop();
        return this;
      }
    }
...
(function() {
  Nali.Model.extend({
    Notice: {
      initialize: function() {
        return this.prototype.prototype.expand({
          Notice: this
        });
      },
      ...
      info: function(params) {
        return this["new"](this.prepare(params)).show('info');
      },
      warning: function(params) {
        return this["new"](this.prepare(params)).show('warning');
      },
      error: function(params) {
        return this["new"](this.prepare(params)).show('error');
      }
    }
  });
  ...
  Nali.extend({
    Audio: {
      ...
      play: function(index) {
        ...
        if ((sound.file != null) && this.supported) {
...
(function() {
  Nali.Model.extend({
    Contact: {
      ...
      activeHistory: [],
      onDestroy: function() {
        if (this.user === this.Application.user) {
          if (this.active) {
            return this.redirect('user');
          }
        } else if (!this.destroyInitiator) {
          return this.Notice.info("" + this.user.name + " больше не хочет общаться с Вами, диалог удален");
        }
      },
    ...
    Dialog: {
      ...
      beforeShow: {
        index: function() {
          ...
          return this.contact().activate().view('index').activeModeOn();
        }
      },
      afterHide: {
        index: function() {
          ...
          return (_ref = this.contact()) != null ? _ref.deactivate().view('index').activeModeOff() : void 0;
        }
      },
      contact: function() {
      ...
      writes: function() {
        return this.view('index').showWrites();
      },
...
(function() {
  Nali.Model.extend({
    Home: {
      forced: true,
      newUser: function(gender, color) {
        return this.query('users.build', {
          gender: gender,
          color: color
        }, (function(_this) {
  ...
  Nali.Model.extend({
    Message: {
      ...
      createMessage: function(params) {
        ...
        return this.Message.create(params);
      }
    }
  });
...
(function() {
  Nali.Model.Notice.expand({
    newmsg: function() {
      return this.Audio.play(this.Application.user.sound);
    }
  });
...
(function() {
  Nali.Model.extend({
    Photo: {
      ...
      cloning: function() {
        this.getter('small', (function(_this) {
          return function() {
            return '/images/photos/small/' + _this.secret + '.jpg';
          };
        })(this));
        this.getter('medium', (function(_this) {
          return function() {
            return '/images/photos/medium/' + _this.secret + '.jpg';
          };
        })(this));
        return this.getter('large', (function(_this) {
          return function() {
            return '/images/photos/large/' + _this.secret + '.jpg';
          };
        })(this));
      },
      ...
      _selectModeOn: function() {
        return this.view('preview').selectModeOn();
      },
      _selectModeOff: function() {
        return this.view('preview').selectModeOff();
      },
      _avatarModeOn: function() {
        return this.view('preview').avatarModeOn();
      },
      _avatarModeOff: function() {
        return this.view('preview').avatarModeOff();
      }
    }
  });
...
(function() {
  ...
  Nali.Model.extend({
    User: {
      ...
      cloning: function() {
        return this.getter('avatarPath', (function(_this) {
          return function() {
            return '/images/avatars/' + _this.avatar + '.jpg';
          };
        })(this));
      },
      beforeShow: {
      ...
      onUpdateHow: function() {
        return this.deactivateSearch();
      },
      onUpdateWho: function() {
        return this.deactivateSearch();
      },
      changeColor: function(_arg) {
        var color;
        color = _arg.color;
        return this.update({
          color: color
      ...
      changeColorDialog: function() {
        var view;
        if ((view = this.view('color')).visible) {
          return view.hide();
        } else {
          return view.show();
        }
      },
      photosDialog: function() {
        return this.show('photos');
      },
      ...
      emailSend: function(_arg) {
        ...
        if (/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,4})+$/.test(email)) {
          ...
          return this.Notice.info('Введите правильный e-mail');
        }
      },
      deleteAccept: function() {
        return this.destroy((function(_this) {
          return function() {
            return _this.logoutAccept();
          };
        })(this));
      },
      logoutAccept: function() {
        this.query('users.logout');
        this.Cookie.remove('token');
        this.Application.user = null;
        return this.redirect('home');
      },
      showSlides: function(slides, start) {
        return this.view('slider').setSlides(slides, start).show();
      },
      toggleSelectPhoto: function(photo) {
        if (__indexOf.call(this.selectedPhotos, photo) >= 0) {
          this.selectedPhotos.splice(this.selectedPhotos.indexOf(photo), 1);
        } else {
          this.selectedPhotos.push(photo);
        }
        return this.trigger('update.selectedPhotos');
      },
      resetSelectedPhotos: function() {
        this.selectedPhotos = [];
        this.trigger('update.selectedPhotos');
        return this;
      },
      sendSelectedPhotos: function() {
        ...
        dialog.sendPhotos(this.selectedPhotos);
        return this.hide('photos');
      },
      deleteSelectedPhotos: function() {
        var _ref;
        if ((_ref = this.selectedPhotos) != null ? _ref.length : void 0) {
          return this.show('deletePhotos');
        } else {
          return this.Notice.info('Выберите фото для удаления');
        }
      },
      deletePhotosAccept: function() {
        ...
        this.resetSelectedPhotos();
        return this.hide('deletePhotos');
      },
      changeAvatar: function() {
        return this.show('photos').avatarModeOn().cancelIsClose = true;
      },
  ...
  Nali.View.extend({
    ContactIndex: {
      ...
      helpers: {
        ...
        userFace: function() {
          if ((this.getOf(this.my.contact, 'avatar')) != null) {
            return '<span class="avatar" style="background-image: url(' + this.my.contact.avatarPath + ')"></span>';
          } else {
            return '<i class="faceicon-' + (this.getOf(this.my.contact, 'gender')) + (this.getOf(this.my.contact, 'image')) + '"></i>';
          }
        }
      },
...
(function() {
  Nali.View.extend({
    DialogIndex: {
      layout: function() {
        return this.Application.user.view('interface');
      },
      onShow: function() {
        this.Application.setTitle('Диалог');
        this.messagesBox = this.element.find('.MessagesIndexRelation');
      ...
      scrollTo: function(message) {
        this.hideWrites();
        return this.messagesBox[0].scrollTop += message.view('index').element.height();
      },
      showWrites: function() {
...
(function() {
  Nali.View.extend({
    HomeIndex: {
      ...
      selectColor: function(_arg) {
        this.color = _arg.color;
        return this.element.find('.gender_buttons').removeClass('red orange yellow green azure blue violet').addClass(this.color).end().find('.logo').addClass('logo_animation');
      },
      selectGender: function(_arg) {
        var gender;
        gender = _arg.gender;
        return this.model.newUser(gender, this.color);
      }
    }
  });
...
(function() {
  Nali.View.extend({
    MessageBuild: {
      ...
      helpers: {
        emoticons: function() {
          var array, code, emoticons, hex, i, _i, _len;
          array = [61, 45, 69, 70, 71, 57, 58, 59, 62, 53, 20, 38, 43, 44, 73, 10, 48, 47, 46, 11, 40, 33, 72, 21, 22, 39, 37, 54, 25, 27, 60, 31, 55, 17, 56, 19, 1, 63, 32, 24, 13, 26, 12, 28, 29, 7, 41, 8, 3, 66, 65, 23, 35, 14, 30, 75, 67, 15, 51, 52, 74, 2, 34, 50, 76, 64, 9, 36, 5, 68, 42, 16, 18, 6, 49, 0, 4];
          emoticons = '';
      ...
      onShow: function() {
        this.textarea = this.element.find('.message textarea');
      ...
      formSubmit: function(event) {
        ...
        return setTimeout(((function(_this) {
          return function() {
            return _this.textarea.val('').trigger('autosize.resize');
          };
        })(this)), 100);
      },
      ...
      pasteEmoticon: function(event) {
        var code, text;
        code = '&#x' + this._(event.target).data('code') + ';';
        text = this._('<textarea>').html(code).text();
        return this.textarea.insertAtCaret(text).trigger('autosize.resize');
      },
  ...
  Nali.View.extend({
    MessageIndex: {
          ...
          if (this.my.text != null) {
            ...
            for (_i = 0, _len = message.length; _i < _len; _i++) {
              ...
              if (__indexOf.call((function() {
                _results = [];
                for (_j = 57344; _j <= 57420; _j++){ _results.push(_j); }
                return _results;
              }).apply(this), code) >= 0) {
                text += '<i>' + sym + '</i> ';
              } else if (code === 10) {
      ...
      onShow: function() {
        ...
        return this.my.dialog.view('index').scrollTo(this.my);
      },
      resizingPhotos: function() {
        var count, element, first, height, i, index, last, lastWidth, lines, matrix, photo, start, startWidth, _i, _j, _k, _len, _ref, _results;
        if (count = this.my.messagephotos.length) {
          ...
          for (index = _k = 0, _len = _ref.length; _k < _len; index = ++_k) {
            ...
            _results.push(element.children[0].style.paddingTop = height);
          }
          return _results;
        }
      }
...
(function() {
  Nali.View.extend({
    PhotoAvatar: {
      ...
      onShow: function() {
        var boxHeight, boxWidth, photoBox;
        photoBox = this.element.find('.photo');
        boxWidth = photoBox.width();
        boxHeight = photoBox.height();
        return this.photo = this._("<img src=\"" + this.my.large + "\" />").appendTo(photoBox).Jcrop({
          aspectRatio: 1,
          minSize: [64, 64],
          boxWidth: boxWidth,
          boxHeight: boxHeight,
    ...
    PhotoPreview: {
      ...
      openOrMode: function(event) {
        ...
        if (this.selectMode) {
          ...
          return this.my.user.toggleSelectPhoto(this.my);
        } else if (this.avatarMode) {
          this.my.user.view('photos').avatarModeOff();
          return this.my.avatar();
        } else {
...
(function() {
  Nali.View.extend({
    UserAccount: {
      layout: function() {
        return this.my.view('settings');
      },
      events: [],
      helpers: {},
      onShow: function() {
        this.Application.setTitle('Аккаунт');
        this.element.closest('.yield')[0].scrollTop = 0;
        return this._('#settingsBar').addClass('active_4');
      },
      onHide: function() {
        return this._('#settingsBar').removeClass('active_4');
      }
    }
  });
...
(function() {
  ...
  Nali.View.extend({
    UserBookmark: {
      ...
      onHide: function() {
        this.Application.setTitle('Настройки');
        return this.Router.changeUrl('user/settings');
      },
      notHide: function(event) {
        return event.stopPropagation();
      }
      ...
      helpers: {
        stylize: function() {
          if (this.getOf(this.my.contacts, 'length')) {
            ...
            return 'show_search_' + (this.my.search ? 'on' : 'off');
          }
        }
      },
      onShow: function() {
...
(function() {
  Nali.View.extend({
    UserInterface: {
      helpers: {
        ...
        userFace: function() {
          if (this.my.avatar != null) {
            ...
            return '<i class="faceicon-' + this.my.gender + this.my.image + '"></i>';
          }
        }
      },
      onShow: function() {
        this.element.find('.ContactsIndexRelation').niceScroll({
          railalign: 'left'
        });
      ...
      onHide: function() {
        return this.element.find('.ContactsIndexRelation').getNiceScroll().remove();
      },
      changeFavicon: function() {
        return this._('#favicon').replaceWith('<link id="favicon" rel="shortcut icon" href="/images/favicons/favicon_' + this.my.color + '.ico" type="image/x-icon">');
      }
    }
  });
  ...
  Nali.View.extend({
    UserPerson: {
      ...
      onShow: function() {
        ...
        this._('#settingsBar').addClass('active_2');
        return this.element.closest('.yield')[0].scrollTop = 0;
      },
      onHide: function() {
        return this._('#settingsBar').removeClass('active_2');
      }
    }
  });
    ...
    UserPhotos: {
      ...
      helpers: {
        selectedCount: function() {
          var count, _ref;
          this.redrawOn(this.my, 'update.selectedPhotos');
          if (count = (_ref = this.my.selectedPhotos) != null ? _ref.length : void 0) {
      ...
      onShow: function() {
        ...
        if (this.photosBox == null) {
          this.photosBox = this.element.find('.PhotosPreviewRelation');
        }
      ...
      selectModeOn: function() {
        if (this.my.photos.length) {
          ...
          this.my.resetSelectedPhotos().photos.selectModeOn();
        } else {
  ...
  Nali.View.extend({
    UserSearch: {
      ...
      onShow: function() {
        ...
        this.element.closest('.yield')[0].scrollTop = 0;
        return this._('#settingsBar').addClass('active_1');
      },
      onHide: function() {
        return this._('#settingsBar').removeClass('active_1');
      },
  ...
  Nali.View.extend({
    UserSettings: {
      ...
      onShow: function() {
        ...
        return this.element.find('.yield').niceScroll({
          railoffset: {
            left: 14
          }
        });
      ...
      onHide: function() {
        ...
        return this.element.find('.yield').getNiceScroll().remove();
      }
    }
  });
...
(function() {
  Nali.View.extend({
    UserSlider: {
      ...
      showSlide: function() {
        return this.slide.html('').append(this._('<img src="' + this.current.large + '" style="max-width:' + this.slideW + 'px;max-height:' + this.slideH + 'px; alt="" />'));
      },
      next: function(event) {
        event.stopPropagation();
  ...
  Nali.View.extend({
    UserSounds: {
      ...
      onShow: function() {
        ...
        this.element.closest('.yield')[0].scrollTop = 0;
        return this._('#settingsBar').addClass('active_3');
      },
      onHide: function() {
        return this._('#settingsBar').removeClass('active_3');
      },
      activateRadio: function() {
